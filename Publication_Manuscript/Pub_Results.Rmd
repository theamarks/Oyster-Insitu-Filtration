...
<<<<<<< HEAD
*O. lurida* had the highest DTW with `r round(NPD_biv_DTW_m2[NPD_biv_DTW_m2$Species=="Ostrea lurida",2],2)` g/m^2^, followed by *M. galloprovincialis* (`r round(NPD_biv_DTW_m2[NPD_biv_DTW_m2$Species=="Mytilus galloprovincialis",2],)` g/m^2^), an unknown *Modiolus* sp. (`r round(NPD_biv_DTW_m2[NPD_biv_DTW_m2$Species=="Unknown mussel",2],2)` g/m^2^), *A. diegensis* (`r round(NPD_biv_DTW_m2[NPD_biv_DTW_m2$Species=="Adula diegensis",2],2)` g/m^2^), and *M. senhousia* (`r round(NPD_biv_DTW_m2[NPD_biv_DTW_m2$Species=="Musculista senhousia",2],2)` g/m^2^) (Figure \ref{Bivalve_DTW_Biomass}).
...
>>>>>>> 0471af8bf4c877d94f9318576ab206ccdafa0b1e
...
## Deanza Mean O.lurida biomass per quadrat
npd_dtw_oly <- NPD_biv_DTW_m2[NPD_biv_DTW_m2$Species=="Ostrea lurida",3]

<<<<<<< HEAD
## Deanza mean Temperature and organic conent 
NPD_mean_wq <- NPD_filter_sum %>% 
  dplyr::summarise(mean_temp = mean(Temp_C_Up),
                   mean_OC = mean(Avg_OC_Ratio, na.rm = T))
...
# estimate clearance rate (L/hr/g) with NPD mean water quality & Gray's model
NPD_oly_cr_est <- estimate_oly_cr_gray(Temp = NPD_mean_wq$mean_temp, 
                                       OC = NPD_mean_wq$mean_OC)
# translate L/hr/g to L/hr/m^2 by adjusting for NPD O. lurida DTW / m^2
NPD_oly_hcr_est <- NPD_oly_cr_est*npd_dtw_oly
```

<<<<<<< HEAD
```{r, Gray_particle_selection}
=======
...
data1 = read.csv("Data/Gill_Area_Measurements.csv",
                 h=T, check.names = F)
data2 = read.csv("Data/Frequency_of_beads_among_individuals_equal_volume_FINAL.csv", 
                 h=T, check.names = F)
data3 = read.csv("Data/Frequency_of_beads_among_individuals_equal_concentration_FINAL.csv",
                 h=T, check.names = F)
...
```{r Part_select_equal_mosaic_plots, echo=FALSE, fig.align="center", fig.asp=0.618, fig.width=6, warning= F, message= F, fig.keep='all', dev='pdf', out.width="70%", fig.cap="Caption Needed From Matt"}
...
### Add code here for Particle seleciton analysis and graph
```
