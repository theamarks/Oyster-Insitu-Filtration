...
knitr::opts_knit$set(root.dir = normalizePath('../'), # set working directory to project level
                     fig.path = "./Publication_Manuscript/Figures")
...
A statistical power analysis determined that the effect size between filtration and control trial HCR was very large (*ES* = `r round(test1_power$d, 2)`), per the criteria in @Sawilowsky2009a, but there was little power (`r round(test1_power$power,2)`) to detect a statistical signal.
...
The residuals of this random forest regression adjusted the HCR values to account for variance caused by water quality variables and site. 
I then compared these adjusted HCRs between filtration and control trials; they were not significantly different (two-sample, one-tail *t*(`r t_test_adj_df`) = `r round(t_test_adj$statistic, digits = 2)`, *p* = `r round(t_test_adj$p.value, digits = 3)`).
A statistical power analysis determined that the effect size of adjusted HCRs between filtration and control trials was still very large (*ES* = `r round(adj_test1_power$d,2 )`) [@Sawilowsky2009a], and with only slightly improved power (`r round(adj_test1_power$power, 2)`) than the unadjusted HCR t-test.
Despite non-significant results between control and filtration trials, statistical power analysis indicated that my limited sample size is inhibiting statistical inference of a very large effect size between oyster habitat HCRs relative to mudflat control HCRs.
...
############### Dr. Nichols Power analysis #######
#n=21
#r2 = 1068382/(1068382+6947629)
#r2

#increase n to 60, r2 stays the same and sumsq should scale by 60/n
#n.new = 21:200
#SSR.new = 1068382*n.new/n
#SSE.new = 6947629*n.new/n
#MSR.new = SSR.new/3
#MRE.new = SSE.new/(n.new-3-1)
#Fa = MSR.new/MRE.new
#Fc = qf(.95,3,n.new-3-1)
#F.ratio = Fc/Fa
#power = 1 - pf(F.ratio,3,n.new-3-1)
#power
#plot(n.new,power,type="l",x.lab="Proposed Sample Size",y.lab = "Power",main="Power analysis if r2 = .133")

#Now we want to hold n constant and toggle r^2.
#Assume sst is constant, change r^2 which gives us new SSR and SSE, then go from there.
#r2.new = seq(.1,1,by=.01)
#SST = 1068382+6947629
#SSR.new = r2.new*SST
#SSE.new = SST-SSR.new
#MSR.new = SSR.new/3
#MRE.new = SSE.new/(n-3-1)
#Fa = MSR.new/MRE.new
#Fc = qf(.95,3,n-3-1)
#F.ratio = Fc/Fa
#power = 1 - pf(F.ratio,3,n-3-1)
#plot(r2.new,power,type="l",xlab="Hypothetical r2",ylab = "Power",main="Power analysis if n=21")
######################
...
#hist(Filter_only_outRmd_data$L_hr_m2, breaks = 25)

# Similar distributions? - I think so
#hist(Filter_only_outRmd_data[Filter_only_outRmd_data$Site == "San Rafael",L_hr_m2], breaks = 25)
#hist(Filter_only_outRmd_data[Filter_only_outRmd_data$Site == "Morro Bay",L_hr_m2], breaks = 25)
#hist(Filter_only_outRmd_data[Filter_only_outRmd_data$Site == "Shellmaker",L_hr_m2], breaks = 25)
#hist(Filter_only_outRmd_data[Filter_only_outRmd_data$Site == "Deanza",L_hr_m2], breaks = 25)

# check data is grouped / ordered by site
#levels(Filter_only_outRmd_data$Site)
