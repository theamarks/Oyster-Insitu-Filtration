...
### ***ANOVA ChlRmd ~ Sal VS ChlRmd ~ log(Sal)***
```{r, fig.height=6, fig.width=6, cache =TRUE}
...
# dataframe with predicted values from log-log Salinity ChlRmd model.
# used to graph model on plot
## Not working - tried adding , predict(model, type="response") to predict in the scale of the response
pred_loglog_Sal_df <- data.frame(pred_Sal = exp(predict(logChlRmdPos_logSal_lm)), 
                                           ChlRmd = Filtration_pos_data$pcnt_Chl_rmvd)

coef(logChlRmdPos_logSal_lm)

logChlRmd_pos_logSal_graph2 <- ggplot(Filtration_pos_data, aes(Sal_ppt_Up, pcnt_Chl_rmvd)) +
  geom_line(data = pred_loglog_Sal_df, aes(x = pred_Sal, y = ChlRmd)) +
  #geom_smooth(formula = log(pcnt_Chl_rmvd) ~ log(Sal_ppt_Up)) +
  geom_point(aes(color = Site)) +
  scale_color_manual(values = Site_colors) +  
  theme_classic() +
  labs(x = 'Salinity Upstream (ppt)',
...
plot(logChlRmd_pos_logSal_graph2)
```
...
### ***ANOVA model test ChlRmd ~ Sal VS log(ChlRmd) ~ log(Sal)***
```{r, fig.height=6, fig.width=6, cache =TRUE}
anova(ChlRmdPos_Sal_lm, logChlRmdPos_logSal_lm)
```

> Warning message: "log(pcnt_Chl_rmvd) removed becaues response differes from model1. How to test if transformation is actually better at predicting data?? 
