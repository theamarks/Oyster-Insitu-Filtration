...
# source functions and output directories 
source("functions.R")
...
Bivalve_Den_data <- Master_analysis_table %>% 
  select(Site, Adula_diegensis_m2, Musculista_senhousia_m2, Mytilus_galloprovincialis_m2, Ostrea_lurida_m2,
         Argopecten_ventricosa_m2, Geukensia_demissa_m2, Crassostrea_gigas_m2, Bivalve_den_m2) %>% 
  unique() # condenses data into unique values

# replace NAs with 0s
Bivalve_Den_data[is.na(Bivalve_Den_data)] <- 0


Bivalve_Den_graph <- ggplot(Bivalve_Den_data, aes(x = Site)) %>% 
