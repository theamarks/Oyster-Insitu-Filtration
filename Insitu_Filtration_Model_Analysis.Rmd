...
```{r LogChlRmdPos_LogSalinity_graph, fig.height=5, fig.width=12, cache=TRUE}
...
pred_loglog_Sal_df <- data.frame(pred_ChlRmd = exp(predict(logChlRmdPos_logSal_lm)), 
                                           Sal = Filtration_pos_data$Sal_ppt_Up)

coef(logChlRmdPos_logSal_lm)

logChlRmd_pos_logSal_graph2 <- ggplot(Filtration_pos_data, aes(Sal_ppt_Up, pcnt_Chl_rmvd)) +
  geom_line(data = pred_loglog_Sal_df, aes(x = Sal, y = pred_ChlRmd)) +
  ...
  labs(x = 'Salinity Upstream (ppt)',
       ...
       title = 'Positive Chl Removed Only',
       subtitle = "Not sure if model x values are appropriate")

grid.arrange(logChlRmd_pos_logSal_graph, logChlRmd_pos_logSal_graph2, nrow = 1)
```
