...
FR_box <- ggplot((Filtration_only_data), aes(Site, L_hr_m2)) +
  ...
  theme_classic() +
  stat_summary(fun.data = n_mean_stats, geom = "text", vjust = 0.2) +
  labs(x = 'Site',
       y = Hab_FR_Label) +
  ylim(-2500, 1300)
...
ggpubr::ggarrange(Chl_RmdPos_TPM_graph, logChlRmdPos_logTPM_graph, 
                  ChlRmdPos_logTPM_graph, ChlRmdPos_TPM2_graph, 
                  ncol = 2, nrow = 2,
...
```{r FRPos vs OC, fig.height=5, fig.width=15, cache=TRUE}
...
TPM_box <- ggplot((Avg_TPM_summary_table), aes(Site, Avg_TPM)) +
  ...
  scale_fill_manual(values = Site_colors) +
  theme_classic() +
  stat_summary(fun.data = n_mean_stats3, geom = "text", vjust = 0.1) + # mean isn't working 
  labs(x = 'Site',
       y = TPM_Label,
...
OC_box <- ggplot((Avg_TPM_summary_table), aes(Site, Avg_OC_Ratio)) +
  ...
  scale_fill_manual(values = Site_colors) +
  theme_classic() +
  stat_summary(fun.data = n_mean_stats3, geom = "text", vjust = 0.0) + 
  labs(x = 'Site',
       y = "Average Organic Content Ratio",
...
> R^2 adj: 0.67, length significant predictor of log(DTW) p-value: 0.000 
...
> R^2 adj: 0.72. p-value: 0.000. Slightly better fit than just y-axis transformation. Interpret the x coefficient as the percent increase in y for every 1% increase in x (log-log)
...
> R^2 adj: 0.49, p-value: x 0.546, x^2 0.146
...
> significant. p-value: 0.000, R^2 adj 0.602, DTW = 0.0313TWW + 0.0139
...
> sig. p-valueL 0.000, R^2 adj: 0.636. Hardly better than y~x
...
> significnt. p-value: 0.00, R^2 adj: 0.845
...
> x sig, x^2 not sig. R^2 adj: 0.607 Not a better fit than y~x
...
> R^2 adj: 0.708, x^2 p-value: 0.002
