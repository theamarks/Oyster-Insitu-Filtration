...
### LM FR- excluding San Rafael logic variable
...
### LM - FR Filter Trials Only
```{r LM_FR_Filter_only, fig.height=15, fig.width=6, cache=TRUE}
# Filtration Trials only - no Negative Controls
...
### LM - Filter Pos ChlRmd - Addative model
```{r LM_PrctChl_pos_addative, fig.height=15, fig.width=6, cache=TRUE}

# Positive Percent Chl removed trials only
LM_PrctChl_pos_add <- lm(Filtration_pos_data$pcnt_Chl_rmvd ~
                         # site logic variables 
                         Filtration_pos_data$Wind + 
                         # Filtration_pos_data$Algae + # no trials with Algae observed
                         # Quantatative variables
                         Filtration_pos_data$Temp_C_Up + 
                         Filtration_pos_data$Sal_ppt_Up + 
                         Filtration_pos_data$avg_m_hr + 
                         Filtration_pos_data$Avg_OC_Ratio)

summary(LM_PrctChl_pos_add)
```

> What does it mean if I descirbe 42 % of the variation, but have no significant factors?

### LM - FR Filter Trials w/ positive Percent Chl removal
```{r LM_PrctChl_positive, fig.height=15, fig.width=6, cache=TRUE}

# Positive Percent Chl removed trials only
LM_FR_pos <- lm(Filtration_pos_data$L_hr_m2 ~
                         # Field notes logic variables
                         Filtration_pos_data$Wind + # Filtration_pos_data$G_upstream + 
                      # Filtration_pos_data$Daylight + Filtration_pos_data$Boat_wake + 
                         # Quantatative variables
...
### olsrr stepwise model
```{r}

FR_all_olsrr <- lm(data = Filtration_only_data, L_hr_m2 ~
                         # site logic variables - SR not included - would break model to include all sites
                         NPD + NPSM + 
                         MB + 
                         # Field notes logic variables
                         Wind + G_upstream +
                         Daylight +
                         Sonde_fell + Boat_wake + 
                         Algae +
                         # Quantatative variables
                         Temp_C_Up + Sal_ppt_Up +
                         Turbidity_NTU_Up + Chl_ug_L_Up +
                         avg_depth_cm + d_bw_sondes_m +
                         avg_m_hr + Avg_OC_Ratio)

ols_step_both_p(FR_all_olsrr)

```

> "Stepwise Regression - Build regression model from a set of candidate predictor variables by entering and removing predictors based on p values, in a stepwise manner until there is no variable left to enter or remove any more."

```{r}
NPD_FR_LM <- lm(data = Filtration_only_data %>% filter(Site == "Deanza"), L_hr_m2 ~ 
                  # Field notes logic variables
                  Wind + Boat_wake + Algae +
                         # Quantatative variables
                         Temp_C_Up + Sal_ppt_Up + Avg_OC_Ratio)

ols_step_both_p(NPD_FR_LM)
