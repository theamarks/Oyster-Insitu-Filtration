...
LM_PrctChl_Filter <- lm(Filtration_only_Analysis$pcnt_Chl_rmvd ~
                         ...
                         Filtration_only_Analysis$NPD + Filtration_only_Analysis$NPSM + 
                         Filtration_only_Analysis$MB + 
                         # Field notes logic variables
                         Filtration_only_Analysis$Wind + Filtration_only_Analysis$G_upstream +
                         Filtration_only_Analysis$Daylight +
                         Filtration_only_Analysis$Sonde_fell + Filtration_only_Analysis$Boat_wake + 
                         Filtration_only_Analysis$Algae +
                         # Quantatative variables
...
### Percent Chl removed X Temp Linear Model (All filtration)
```{r}
Prt_Chl_temp_lm <- lm(data = Filter_data, pcnt_Chl_rmvd ~ Temp_C_Up)
summary(Prt_Chl_temp_lm)
```

```{r}
Prt_Chl_temp_pos_lm <- lm(data = Chl_Rmd_data_pos, pcnt_Chl_rmvd ~ Temp_C_Up)
summary(Prt_Chl_temp_lm)
```
...
> stat_poly_eq() draws different equation doesn't match the line drawn... Why???! Should be y = 4.166e-03 + 8.270e-05 x^2 stat_poly_eq() "It supports only linear models fitted with function lm()"
...
### Exponential model
```{r Quad_model_DTW_length_4}
#Quadratic model using length and length^2
Oly_DTW_Exp <- lm(data = NPD_oly_outRmd,
...
```{r}
NPD_oly_outRmd %<>% 
  mutate(Exp_model = unname(predict(Oly_DTW_Exp)))
# Are these predicted values correct? all negative and make my graph crazy

Oly_DTW_Exp_graph <- ggplot(NPD_oly_outRmd, aes(length_mm, tissue_dry_weight_g)) +
  geom_point() +
  geom_line(aes(length_mm, log(Exp_model))) +
  theme_classic() +
  labs(title = 'Olympia Oyster log(DTW) predicted by Shell Length - Newport, CA') +
  stat_poly_eq(aes(label = ..adj.rr.label..), formula = logy_x_formula, 
               parse = TRUE,
               label.x = "right",
               label.y = "bottom") 
plot(Oly_DTW_Exp_graph)
```

> What is happening with this graph? R^2 is much better 0.67, but why is the line crazy? 
...
Oly_TWW_DTW_rm_out <- ggplot(Oly_shell_DTW_rm_out,
...
```{r}
Bivalve_Biomass_Data <- Bivalve_Den_data %>% 
