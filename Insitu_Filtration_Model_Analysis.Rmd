...
```{r LM_PrctChl_Filter_only, fig.height=10, fig.width=6, cache=TRUE}
...
> Only 3 degrees of freedom, no significant predictors of ChlRmd. Try adding in log transformed salinity
...
> Newport Shellmaker TRUE was a predictor of ChlRmd... indication of site effect? Really low R^2 value - 0.116

### ***FR - Filtraiton Trials Only***
```{r LM_FR_Filter, fig.height=10, fig.width=6, cache=TRUE}
# Filtration Trials only - no Negative Controls
...
> 3 degrees of freedom, no significant parameters in full model. Try log transformed salinity next.

### ***Filtration Only ~ FR Multiple Regression***
```{r, fig.height=10, fig.width=6, cache=TRUE}
# detailed multiple regression summary - ANOVA results
ols_regress(LM_FR_Filter)
```

### ***Filtration Only ~ FR Stepwise Multiple Regression***
...
> NP Shellmaker True signature? low R^2 fit. 
...
### ***Deanza- Chl Rmd - Filtration Trials Stepwise Multiple Regression***
```{r LM_ChlRmd_NPD_filter, cache=TRUE}
LM_ChlRmd_NPD_filter <- lm(data = Deanza_Filter_data, pcnt_Chl_rmvd ~
                         # site logic variables - SR not included - break model to include all sites
                         NPD + NPSM + MB + 
                         # Field notes logic variables
                         Wind + G_upstream +  Daylight +  Sonde_fell + Boat_wake + Algae +
...
ols_step_both_p(LM_ChlRmd_NPD_filter)
```

> Lots of errors, not enough data to run all possible model combinations. Salinity shows as single predictive parameter. 

### ***FR - Deanza - Filtration Trials***
```{r LM_FR_NPD_filter, cache=TRUE}
...
> Not enough data to run all combinations. Turbidity and Boat wake 
...
```{r, Filter by TPM filter only, fig.height=5, fig.width=14, cache=TRUE}
...
> Log transforming axes Doesn't not help R^2 or significance.
...
### ***Filtration vs OC***
```{r FR vs OC, fig.height=5, fig.width=14, cache=TRUE}
...
> Significant Avg_OC_Ratio p-value: 0.04
...
logChl_RmdPos_logOC_graph <- ggplot(Filtration_pos_data, 
  ...
  ggpmisc::stat_poly_eq(aes(label = paste("atop(",..eq.label.., ",",..adj.rr.label.., ")", sep ="")),
               ...
               label.x = "right",
               label.y = "bottom")
plot(logChl_RmdPos_logOC_graph)
```

### ***logChlRmd Pos by logOC Linear Model***
```{r, fig.height=6, fig.width=6, cache =TRUE}
...
### ***Temperature***
```{r Filter_Temp, fig.height=10, fig.width=14, cache=TRUE, echo=FALSE}
...
```{r, fig.height=6, fig.width=6, cache =TRUE}
NPD_FRPos_temp_lm <- lm(data = Deanza_PosFilter_data, L_hr_m2 ~ Temp_C_Up)
...
### ***Filtration - Salinity***
```{r Filter_Salinity, fig.height=10, fig.width=14, cache=TRUE, echo=FALSE}
...
> log transformation of x-axis doesn't change significance. p-value: 0.072

### ***Log ChlRmd Pos - Log Salinity***
```{r LogChlRmdPos_LogSalinity_graph, fig.height=5, fig.width=7, cache=TRUE}
...
```{r Filter_Chl, fig.height=10, fig.width=14, cache=TRUE, echo=FALSE}
...
> not significant. No obvious transformation to make. p-value: 0.089
...
> Log transforming x-axis didn't improve fit. p-value: 0.242
...
> Log transforming both axes didn't improve fit. p-value: 0.236

Turbidity
=====================================

### ***Filtration - Turbidity***
```{r Filter_Turbidity, fig.height=10, fig.width=14, cache=TRUE, echo=FALSE}
...
ols_regress(NPD_ChlRmd_Turb_lm)
```
...
### ***Deanza FR Pos ~ log Turbidity LM***
```{r, fig.height=6, fig.width=6, cache =TRUE}
NPD_FRPos_logTurb_lm <- lm(data = Deanza_PosFilter_data, L_hr_m2 ~ log(Turbidity_NTU_Up))
ols_regress(NPD_FRPos_logTurb_lm)
```

> Worse fit than regular model. p-value: 0.103, R^2: 0.526

### ***Deanza log FR Pos ~ log Turbidity LM***
```{r}
NPD_logFRPos_logTurb <- ggplot(Deanza_PosFilter_data, aes(log(Turbidity_NTU_Up), log(L_hr_m2))) +
  geom_smooth(method=lm, se=TRUE, level=0.95, color = 'grey50') +
  geom_point() +
  theme_classic() +
  labs(x = expression(paste(log[n], ' Turbidity Upstream (NTU)')),
       y = expression("Log"[n]*" Habitat Filtration Rate (L/hr/m"^2 * ")"),
       title = 'Deanza - Positive Filtration Trials') +
  ggpmisc::stat_poly_eq(aes(label = paste("atop(",..eq.label.., ",",..adj.rr.label.., ")", sep ="")),
               formula = x_y_formula,
               parse = TRUE,
               label.x = "right",
               label.y = "bottom")
plot(NPD_logFRPos_logTurb)

```

### ***Deanza log FR Pos ~ log Turbidity LM***
```{r, fig.height=6, fig.width=6, cache =TRUE}
NPD_logFRPos_logTurb_lm <- lm(data = Deanza_PosFilter_data, log(L_hr_m2) ~ log(Turbidity_NTU_Up))
ols_regress(NPD_logFRPos_logTurb_lm)
```

> Significant Model. p-value: 0.014, R^2: 0.816
...
> R^2 0.72. p-value: 0.000. Slightly better fit than just y-axis transformation
...
NPD_bivalve_biomass_data <- fread(file.path(Density_directory,"Insitu_Filter_NPD_bivalve_biomass_data.csv"))

# what are the unique species in dataset
unique(NPD_bivalve_biomass_data$species)

NPD_bivalve_biomass_data <- NPD_bivalve_biomass_data %>% 
  mutate(species = ifelse(grepl("adula", ignore.case = T, species), "Adula_diegensis",
                          ifelse(grepl("muscul.", ignore.case = T, species), "Musculista_senhousia",
                                 ifelse(grepl("myt", ignore.case = T, species), "Mytilus_galloprovincialis",
                                        ifelse(grepl("^.lurida",ignore.case = T, species), "Ostrea_lurida",
                                               ifelse(grepl("mussel",ignore.case = T, species), "Unknown_mussel",
                                                       "Pattern Not Found")))))) %>% 
  filter(!species %in% c("Pattern Not Found"))

NPD_bivalve_biomass_graph <- NPD_bivalve_biomass_data %>% 
  ggplot(aes(x = length_mm, y = tissue_dry_weight_g, group = species)) +
  geom_point() +
  facet_wrap(~species)

NPD_bivalve_biomass_graph

# Determine what quadrat samples have missing data (NAs)
NPD_biv_bio_summary <- NPD_bivalve_biomass_data %>% 
  group_by(excavation_sample) %>% 
  summarize(avg_DTW = mean(tissue_dry_weight_g))


# Q10, Q4, Q8 are missing DTW data. remove from dataset
NPD_bivalve_biomass_data <- NPD_bivalve_biomass_data %>% 
