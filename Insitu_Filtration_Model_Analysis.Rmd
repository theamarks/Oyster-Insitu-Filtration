...
Filter_data <- arrange(transform(Filter_data, Site = factor(Site,levels = site_by_lat)), Site)

Site_colors <- c("San Rafael" = ,
                 "Morro Bay" =,
                 "Shellmaker" = ,
                 "Deanza" = )
#Oy <- arrange(transform(Oy, State = factor(State,levels = order_by_state)), State)
