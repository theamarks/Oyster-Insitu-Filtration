...
TPM_summary_table <- TPM_data %>% 
  mutate(TPM_mg_L = ((dry_weight_corrected_g - filter_weight_corrected_g) * 1000 / sample_volume_ml), # grams to mg
         PIM_mg_L = TPM_mg_L - ((ash_weight_corrected_g - filter_weight_corrected_g) * 1000 / sample_volume_ml),
...
Avg_TPM_summary_table <- TPM_summary_table %>% 
  ...
  summarize(Avg_TPM = round(mean(TPM_mg_L), 4),
            Avg_PIM = round(mean(PIM_mg_L), 4),
            Avg_POM = round(mean(POM_mg_L), 4),
...
Master_analysis_table <- filtration_summary_table %>% 
  ...
  dplyr::select(-c("estimate", "conf.high", "sample_type","Temp_C_Down", "SpCond_mS_cm_Down",
