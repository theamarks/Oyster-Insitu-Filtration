...
```{r Std_names_plot_raw_chl, fig.height=5, fig.width=13.5}
...
```{r Manual_Sbs_Corr_Plot_Chl_FR_Clac, fig.height=5, fig.width=13.5}
######################################################################
...
SBS - Correction & Uncertainty
=====================================

###
```{r Sbs_Correction_Uncertainty, echo=FALSE}
######################################################################
...
for(file in 1 : length(all_files))
{
  ...
  # Calculate Error Stats for individual day
  one_file = calculateErrorStats(one_file, file_name, manual_correction_df,
                                error_directory)
  
  # Plot SBS Density with stats table
  plot(createSbsDensityPlot(one_file, file_name, corrected_graphs_directory))
  ...
  {  
    ...
    one_file = calculateErrorStats(one_file, file_name, manual_correction_df,
                                    error_directory)
    sbs_stats_summary = rbind(sbs_stats_summary, one_file) 
  }
...
```{r Plot_V_paired_Chl_diff, fig.height=5, fig.width=10}
######################################################################
