...
######################################################################
  ...
  if(nrow(one_file) > 0)
  {  
    one_filtration_summary = createFiltrationSummary(one_file, file_name)
...
```{r TPM_Density, echo=FALSE, cache=TRUE}
...
Master_analysis_table <- filtration_summary_table %>% 
  ...
  select(-ends_with("Down"), -c("estimate", "conf.high", "method", "alternative", "sample_type")) %>% 
  # Add in site simple variables for regression
  mutate(SR = ifelse(grepl("San Rafael", Master_analysis_table$Site), T, F),
         NPD = ifelse(grepl("Deanza", Master_analysis_table$Site), T, F),
         NPSM = ifelse(grepl("Shellmaker", Master_analysis_table$Site), T, F),
         MB = ifelse(grepl("Morro Bay", Master_analysis_table$Site), T, F))

fwrite(Master_analysis_table, file.path('./Data', "Master_Filtration_analysis_table.csv"))
```

```{r Species_Density_Graphs, eval=FALSE, include=FALSE, cache=TRUE}
