...
```{r}
######################################################################
# Create SBS Stats - Uncertainty Table
######################################################################
...
for(file in 1 : length(all_files))
{
  ...
  if(nrow(one_file) > 0)
  {  
    Sbs_stats_summary_table = rbind(filteration_summary_table, one_file)
  }
  # save the sbs STATS table
  fwrite(Sbs_stats_summary_table, file.path(error_directory, file_name))
