...
Bar_up_down_data <- Master_analysis_table %>%
  ...
  mutate(x_labels = ifelse(Experiment == 'Filtration',
                           Date,
                           paste(Bar_up_down_data$Date, '*', sep = ' '))) %>% 
  mutate(Site_exp_date = paste(Bar_up_down_data$Site, Bar_up_down_data$Experiment,
                                     Bar_up_down_data$Date,
                                     sep = ' ')) %>% 
  mutate(Exp_Date = paste(Bar_up_down_data$Experiment, Bar_up_down_data$Date, sep = ' '))
...
(Chl_bar_graph_NPD <- ggplot(data = NPD_bar_data,
                            aes(x = Exp_Date, y = Chl_ug_L, fill = Position)) +
    geom_bar(stat = "identity", position = "dodge", scales = "free") +
    geom_bar(data = (Bar_up_down_data %>%
                       filter(!Experiment %in% c('Filtration'))),
             color = "grey22",
             stat="identity",
