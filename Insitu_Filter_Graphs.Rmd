...
(FR_OC_NPSM_graph <- ggplot(Master_analysis_table %>% 
  ...
  geom_smooth()
```
Filtration Rate (L/hr/m^2) predicted by Water Quality
```{r FR vs Water Quality, echo=FALSE}
# Temperature
(FR_Temp <- ggplot(Master_analysis_table %>% 
                     filter(Experiment == "Filtration"),
                   aes(Temp_C_Up, L_hr_m2, color = Site)) +
  geom_point() +
  theme_classic() +
...
(FR_Temp_bestfit <- ggplot(Master_analysis_table %>% 
                     filter(Experiment == "Filtration"),
                   aes(Temp_C_Up, L_hr_m2)) +
  geom_point() +
  theme_classic() +
  labs(x = 'Temperature Upstream (C)',
       y = 'Clearence Rate (L/hr/m^2)',
...
# Salinity
(FR_Sal <- ggplot(Master_analysis_table %>% 
                         filter(Experiment == "Filtration"),
                       aes(Sal_ppt_Up, L_hr_m2, color = Site)) +
  geom_point() +
  theme_classic() +
...
(FR_Sal_bestfit <- ggplot(Master_analysis_table %>% 
                         filter(Experiment == "Filtration"),
                       aes(Sal_ppt_Up, L_hr_m2)) +
  geom_point() +
  theme_classic() +
  labs(x = 'Salinity Upstream (ppt)',
       y = 'Clearence Rate (L/hr/m^2)',
...
# Chlorophyll 
(FR_Chl <- ggplot(Master_analysis_table %>% 
                            filter(Experiment == "Filtration"),
                          aes(Chl_ug_L_Up, L_hr_m2, color = Site)) +
  geom_point() +
  theme_classic() +
...
(FR_Chl_bestfit <- ggplot(Master_analysis_table %>% 
                          filter(Experiment == "Filtration"),
                        aes(Chl_ug_L_Up, L_hr_m2)) +
  geom_point() +
  theme_classic() +
  labs(x = 'Chlorophyll Upstream (ug/L)',
       y = 'Clearence Rate (L/hr/m^2)',
...
# Turbidity
(FR_Turbidity <- ggplot(Master_analysis_table %>% 
                            filter(Experiment == "Filtration"),
                          aes(Turbidity_NTU_Up, L_hr_m2, color = Site)) +
  geom_point() +
  theme_classic() +
...
(FR_Turbidity_bestfit <- ggplot(Master_analysis_table %>% 
                                    filter(Experiment == "Filtration"),
                                aes(Turbidity_NTU_Up, L_hr_m2)) +
  geom_point() +
  theme_classic() +
...
(Oly_shell_DTW <- ggplot(NPD_oly, aes(length_mm, tissue_dry_weight_g)) +
  ...
  geom_smooth()
# Looks like data error / outlier accidental 1. instead of 0. Remove?
