...
```{r load_packages, include=FALSE, eval=T}
################### Packages ###########################
library(data.table)
library(magrittr)
library(dplyr)
...
All_data <- data.table::fread(file.path('./output/4_Filtration_Calculations/Filtration_Summary', "Master_Filtration_analysis_table.csv"))
Avg_TPM_summary_table <- data.table::fread(file.path('./output/5_TPM_OC_Summary', "Avg_TPM_summary_table.csv"))
Density_directory = "./Data/bivalve_density_community"
NPD_bivalves = data.table::fread(file.path(Density_directory, "Insitu_Filter_NPD_bivalve_biomass_data.csv"))
Sbs_correction_data = data.table::fread(file.path('./output/6_Sbs_Corrections_Summary', "sbs_correction_summary.csv"))
```
...
# values for map
site_names_all <- c("Yaquina", "San Rafael", "Morro Bay", 
                    "Shellmaker", "Deanza", "San Diego")
my_palette <- viridis(option = "C", n = 6, end = 0.9)
# show palette
#show_col(viridis_pal(option = "C", end = 0.9)(6))
Site_colors_all <- setNames(my_palette, site_names_all)
# values for HCR figure
hcr_colors <- setNames(my_palette[2:5], site_names_all[2:5])
# values for DTW
dtw_colors <- rev(setNames(my_palette[c(1,5,6)], site_names_all[c(1,5,6)]))
...
```{r map_figre, include=TRUE, fig.width = 6, fig.height= 6, fig.asp=1}
source(file.path("./Publication_Analysis/scripts/04_map_sites.R"))
base_map
```
...
Filtration trials across sites were not distributed equally (Table), Deanza had more than twice the amount filtration trials (*N* = 9) as San Rafael (*N* = 4), Morro Bay (*N* = 4), and Shellmaker (*N* = 4).

## Habitat Clearance Rates 

```{r Random_Forest_DrNichols, include=F, eval=T}
source(file.path("./Publication_Analysis/scripts/01_random_forest.R"))
```


```{r Random_Forest_Table, include=T, eval=T}
print.xtable(output_table)
...
FR_box
 ...
 ggsave(FR_box, 
        height=3.09, width=5, dpi = 600, 
        device = "eps",
        filename = file.path("./Publication_Analysis/figures/", paste0("HCR_boxplot_",Sys.Date() ,".eps")))
...
# code to construct summary table. table refined in Overleaf. Does not include standard deviation used in publication. SD is in Master_anaysis_table
...
Final_table <- Master_analysis_table %>%
  mutate(Date = mdy(Date),
         ...
         Experiment = ifelse(grepl("Neg Control*", 
                                   Master_analysis_table$Experiment), 
                             gsub("Neg Control*", 
                                  "Ctrl", Master_analysis_table$Experiment), 
                             "Fltr"),
         L_hr_m2 = round(L_Hr_m2, digits = 2),
         L_hr_m2_sd = round(L_Hr_m2_sd, digits = 2),
       #  pcnt_Chl_Rmvd = signif(pcnt_Chl_Rmvd, digits = 3), 
         Temp_C_up = round(Temp_C_up, digits = 2),
         Sal_ppt_up = round(Sal_ppt_up, digits = 2),
         Turbidity_NTU_up = round(Turbidity_NTU_up, digits = 2),
         Avg_TPM_mg_L = round(Avg_TPM_mg_L, digits = 2),
         Avg_OC_Ratio = round(Avg_OC_Ratio, digits = 2),
         Chl_ug_L_up = round(Chl_ug_L_up, digits = 2),
         Chl_ug_L_down = round(Chl_ug_L_down, digits = 1),
         avg_depth_m = round(avg_depth_m, digits = 2),
         d_bw_sondes_m = round(d_bw_sondes_m, digits = 2),
         avg_m_sec = round(avg_m_hr/3600, 2)) %>%
  filter(Experiment == "Fltr") %>% 
  dplyr::select(Site, Date, Tide, L_hr_m2, L_hr_m2_sd, Chl_ug_L_up, 
                Chl_ug_L_down, Temp_C_up, Sal_ppt_up, 
...
source(file.path("./Publication_Analysis/scripts/02_particle_selection.R"))
```

mosaic plot code not up to date with publication version. 
...
```{r Oly_DTW_Shell_height_setup, include=FALSE}
...
## This code chunk needs "Oly_DTW_Shell_height_setup" chunk above 
...
SD_Oly_SL_DTW <- read_csv(file.path("./Data/bivalve_density_community/Langevin_SD_O_lurida_length_weight_data.csv"))
SD_Oly_SL_DTW %<>% 
...
```{r Oly_DTW_SH_analysis}
################ Builds models for each O. lurida population 
source(file.path("./Publication_Analysis/scripts/03_dtw_shell_height.R"))

############### Statistical comparison between each model
source(file.path("./Publication_Analysis/scripts/dtw_model_comp.R"))
```

```{r Oly_DTW_SH_figure, include=T}
################# Graph with Deanza, Yaquina & San Diego DTW x Shell length

# order factor levels
Oly_combo_DTW_SL <- Oly_combo_DTW_SL %>% 
  #arrange(Site)
  arrange(factor(Site, levels = c("San Diego","Deanza","Yaquina")))

# graph
Combo_Oly_shell_DTW_figure <- ggplot(Oly_combo_DTW_SL, aes(length_mm, tissue_dry_weight_g)) +
  geom_point(aes(color = Site, shape = Site)) +
  scale_color_manual(labels = c("Deanza, Newport Bay, CA",
                                "San Diego Bay, CA",
                                "Yaquina Bay, OR"),
                     values = dtw_colors,
                     na.translate = TRUE, drop = FALSE) + 
  scale_shape_manual(labels = c("Deanza, Newport Bay, CA",
                                "San Diego Bay, CA",
                                "Yaquina Bay, OR"),
                     values = c("Deanza" = 16,
                                "San Diego" = 6,
  ...
  ylim(0,1)+
  xlim(0,80) +
  labs(x = 'Shell Height (mm)',
       y = 'Dry Tissue Weight (g)')+
  geom_line(data=temp1,aes(x.seq,y.seq1), color = dtw_colors[1]) + #9986A5
  geom_line(data=temp2,aes(x.seq,y.seq2), color = dtw_colors[2]) +
  geom_line(data=temp3,aes(x.seq,y.seq3), color = dtw_colors[3]) #20A386FF

# test export proportions
#quartz(h = 3.09, w = 5)
Combo_Oly_shell_DTW_figure

 ggsave(Combo_Oly_shell_DTW_figure, 
        height=3.09, width=5, dpi = 600, 
        device = "eps",
        filename = file.path("./Publication_Analysis/figures/",
                             paste0("Oly_DTW_SL_allsites_",Sys.Date() ,".eps")))
#fig.asp=0.618
# height=3.708, width=6
...
```{r Estimate_filter_w_other_studies}
...
# area m^2 of deanza oyster bed (20m x 1m)
npd_area <- 20

# convert biomass from g/m^2 to g
npd_dtw_oly_g <- npd_dtw_oly*npd_area

# Mean Seston Organic Content - Deanza
npd_OC <- mean(NPD_TPM_OC$Avg_OC_Ratio)

# Mean Temperature - Deanza
npd_temp <- mean(Filter_only_outRmd_data[Filter_only_outRmd_data$Site == "Deanza", "Temp_C_up"])

# estimate specific/standardized clearance rate with NPD biomass & Gray's model (L/hr/g)
NPD_oly_cr_est <- estimate_oly_cr_gray(npd_temp, npd_OC)

# estimate total clearance rate for all oysters (L/hr)
NPD_oly_cr_total <- estimate_oly_cr_gray(npd_temp, npd_OC)*npd_dtw_oly_g

# estimate L/hr/m^2
NPD_oly_lhrm2 <- estimate_oly_cr_gray(npd_temp, npd_OC)*npd_dtw_oly
