...
I took the mean of temperature, salinity, turbidity of the upstream sonde during the filtration trail for analyses, and the mean of Chl $\alpha$ at both upstream and downstream sondes for analyses. 
...
All data, in filtration, control, and side-by-side trails, was reviewed for extreme Chl $\alpha$ and turbidity spikes indicating disturbances to the trail. 
...
The mean weight difference between ashed filters and washed only filters was 0.5\%, which I used to corrected the filter pre-weights (Equation \ref{Eq:TPM}, \ref{Eq:PIM}).
In addition, control filters were run with every sampling batch, distilled water was filtered, and the filter was stored and processed the same as samples.
The difference between the pre-weight of the control filters and the dried and ashed weight of the control filters represented processing contamination; I used this difference to correct filter dry and ashed weights.
...
I paired upstream and downstream Chl $\alpha$ measurements in the experiment and control trials to determine how Chl $\alpha$ concentrations changed over the habitat.
...
Next, I calculated percent Chl $\alpha$ removal [$Chl_{rmd}$, Equation \ref{Eq:PrctChl}; @Grizzle2008] and habitat clearance rate [Equation \ref{Eq:Lhrm2}; @Milbrandt2015] using average Chl $\alpha$ concentrations. 
...
Two field days (Morro Bay 2019-05-18 and Deanza 2018-10-25; Table 1) from my data set are missing TPM and OC values, representing `r round((4/336)*100, digits = 0)`% of these values.
...
This comparison showed if any instrument corrections obfuscated the experimental signal.
...
Because my data set was small, extreme values likely influenced my initial confidence intervals; therefore I created a second linear model with the extreme values removed and 98\% confidence intervals to identify additional extreme values to remove from the analysis.

To determine if there was a difference between control and filtration trial HCR, I first used a two-sample t-test.
To examine the effects of water quality and site variables, I fit a random forest regression (including all filtration and control trials) to HCR including temperature, salinity, turbidity, TPM, OC, and site [randomForest R package @Liaw2002].
