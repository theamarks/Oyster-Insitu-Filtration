...
The oyster beds measure 20 m x 1 m x 0.25 m and were constructed with coconut coir bogs filled with clean, commercially-grown *C. gigas* and *Mytilus galloprovincialis* mussel shell. 
...
In the side-by-side trials, I placed the sondes adjacent to one another to compare the sensors' Chl $\alpha$ measurements in the same mass of water for approximately 10 minutes.
...
## Seston Content
I determined seston total particulate matter (TPM), particulate inorganic matter (PIM), particulate organic matter (POM), and organic content (OC) gravametrically.
...
The travel time was subtracted from the timestamps of each downstream measurement to pair with upstream measurements; unpaired measurements were discarded. 
...
Next, I examined the signal-to-correction relationship between side-by-side trial Chl $\alpha$ difference (correction) and corresponding experiment tiral Chl $\alpha$ difference (signal).
This comparison showed if any instrument corrections obfuscate the experimental signal.
I used the 95\% confidence intervals of the signal-to-noise linear model to remove extreme values outside of the intervals.
...
Importantly, note that one out of three extreme values eliminated corresponded with Chl $\alpha$ concentration near the sensor's detection limits (± 0.1 \\mu/L), the other two extreme values correspond with low Chl $\alpha$ concentrations (Table 1).
...
A second two-sample t-test comparing control and filtration trials was adjusted by the random forest model regression residuals, controling for the effects of water quality and site.
...
I chose to use a random forest regression over a multiple linear regression for several resasons. 
...
The final output (forest) is the average of the 2,000 individual trees, which corrects for the individual trees' weaknesses [i.e. emsamble learning @Sagi2018].
