...
This leaves a substancial knowedge gap for better understanding the filtration functions that restored *O. lurida* habitat may provide, and making direct comparisons to previous research findings.
...
My measurements show at least one trial with negative HCR at each site (Figure \ref{FR_Site_boxplot}); a result of higher Chl $\alpha$ concentrations downstream than Chl $\alpha$ concentrations upstream.
...
~ Talk about negative values in control trials ~
...
Bivalve filter-feeding has a greater influence on water column when the water is shallow, whereas the filtration signal can be drowned out in deeper water [@Wheat2013]. 
High water velocities can re-suspend sediments into the water column [@Dame1985; @Widdows1998; @Harsh1999] and also cloud the detection of filtration signals.
...
Considering the extremely low *O. lurida* biomass at San Rafael compared to Deanza, one would not expect San Rafael to have a similar range of HCRs (Figure \ref{FR_Site_boxplot}). 
...
Approximately `r round((nrow(Filter_only_outRmd_data %>% filter(L_hr_m2 > 157)) / nrow(Filter_only_outRmd_data)) * 100)` percent of my trials exceed their largest value (Table 1).
