...
To date, no studies have incorporated the effects of other filter-feeders and complex environmental conditions on Olympia oyster filtration functions.
...
These findings suggest that previously measured (***Citations***) clearance rate differences between *O. lurida* and *C. gigas* may not accurately estimate ecosystem filtration functions of *O. lurida* restoration and *C. gigas* aquaculture cecause they do not take into account *O. lurida* as a foundation species that filters alongside other bivalves in dynamic environmental conditions. 
...
According to the all-season model produced from these measurements and the biomass of *O. lurida* at Deanza (1.36 gDTW m^-2^), HCR should be 1.13 L hr^-1^m^-2^ usingh average OC and temperature from Deanza trials.
...
My HCRs across all sites ranged from -242 to 845 L hr^-1^m^-2^ (Table 1), which encompasses the HCRs range from @Milbrandt2015 but 40\% of trials far exceed their largest values.
...
First, random forest regressions are non-linear models and previous research shows that temperature, salinity, and TPM had non-linear relationship with *O. lurida* and *C. gigas* clearance rates.
Second, random forest regressions work with missing data, and I had two trials with missing TPM and OC values because of a processing error. 
Third, multiple linear regressions can extrapolate predicited values, whereas random forest regressions can not extraploate outside the training data set. 
The random forest model I fit is a collection of 2,000 individual decision trees.
Each tree is prone to overfitting the training data, but the final output is an the average of the 2,000 individual trees, correcting for the individual overfitting.
Fourth, I had a large number of variables relative to observations, limiting my degrees of freedom in a multiple linear regression.
