...
Based on single-species clearance rate studies, on would expect *C. gigas* aquaculture to outperform *O. lurida* restoration in filtration functions.
I found evidence that *in situ* HCRs between native oyster restoration and introduced oyster aquaculture are likely more similar than estimated by previous studies.
Although my study only includes a single *C. gigas* aquaculture site, three *O. lurida* restoration sites in two bays north and south of Morro Bay set up a comparison fairly robust to restoration site differences, albeit not perfect.
My evidence is generated from limited data, however, power analysis of HCR by site further points to a very small effect size and corroborates my findings that *C. gigas* aquaculture in no way "stands out" from *O. lurida* restoration.
Furthermore, *O. lurida*'s filtration functions on a bay-wide spatial scale may dramatically increase when spatially-explicity hydrodynamics are accounted for [@Gray2019a].
...
Thus, bivlave species biomass represents the proportional contribution that a species makes to the overall HCR.
...
This leaves a substancial knowedge gap for better understanding the effects of bivalve community composition on restored *O. lurida* habitat filtration fuctions, and making direct comparisons to previous research findings.
Future studies will benefit from a suite of models that predict species-specific bivalve biomass from shell length measurements, allowing monitoring efforts to measure bivalve shell length without disturbing the habitat to estimate filter feeders biomass.
...
I found that temperature was the most important variable (23%) in predicting HCRs with my random forest model (*R*^2^ = 0.64), aligning with previous bivalve feeding studies.
...
My results provide the first snapshot of habitat clearance rates of oyster habitats in California, and shows that *in situ* upstream-downstream methods can detect filtration signals in this application.
...
My research represents the first direct measurements of restored *O. lurida* habitat filtration functions, and although highly variable, they show that restored *O. lurida* habitat is a valuable contributor to filtration functions within California bays.
