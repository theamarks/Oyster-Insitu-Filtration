...
To date, no studies have incorporated the effects of other bivalve filter-feeders and true environmental conditions on the filtration functions of Olympia oyster habitat.
My results indicate that the habitat clearance rate (HCR) between *C. gigas* long-line aquaculture and restored *O. lurida* habitat are similar when measured in the field; inherently including all bivalve filtration contributions and their responses to dynamic field conditions.
...
Water flow velocity and depth constrained my field days and deployment duration, resulting in filtration function snap shots.
...
My HCRs across all sites ranged from -242 to 845 L hr^-1^m^-2^ (Table 1), which encompasses the HCRs range from @Milbrandt2015 but 40\% of my trials far exceed their largest values.
...
I chose to fit a random forest regression to all filtration trials HCRs for over a multiple linear regression for several resasons. 
First, random forest regressions are nonparametric models that learn nonlinearity relationships without explicitly modeling them [@Gromping2009].
This works well for my data as previous research shows that temperature, salinity, and TPM had non-linear relationship with *O. lurida* and *C. gigas* clearance rates.
...
Third, multiple linear regressions can extrapolate predicited values, whereas random forest regressions can not extraploate outside the training data set [@Ellis2016; @Hengl2018]. 
The random forest model I fit is a collection of 2,000 individual decision trees.
Each tree is prone to overfitting the training data, but the final output is an the average of the 2,000 individual trees, correcting for the individual overfitting [@Sagi2018].
Fourth, I had a large number of variables relative to observations which limiting my degrees of freedom in a multiple linear regression, random forests work well with this kind of data structure [@Gromping2009].
Lastly, using all the same variables (temperature, salinity, turbidity, TPM, OC, and site) a random forest model fit HCRs better (*R*^2^ = 0.62) than a multiple linear regression (*R*^2^ = 0.43).
...
My results provide the first snap shot habitat clearance rates of four unique sites in California, and shows that in situ upstream-downstream methods can detect filtration signals in this application.
...
* Future directions? 
  + Talk about adding spatial component @Grizzle2018 or @Gray2019a?
* Tidal submersion time
  + @Bayne1988a - "Intertidal individuals do not compensate
for periods of aerial exposure by increasing their rate of feeding during immersion. 
  + Higher intertidal may have more access to more organic matter suspended with incoming tide.
