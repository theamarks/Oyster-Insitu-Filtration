...
```{r ChlRmd_Site_boxplot, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align= "center", fig.show="hold", fig.asp=0.618, out.width="70%", message= F, fig.cap="The percent chlorophyll removed of all filtration trials across sites. \\label{ChlRmd_Site_boxplot}"}
...
```{r FR_Site_boxplot, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.show="hold", fig.align= "center", fig.asp=0.618, out.width="70%", message= F, fig.cap="The habitat clearance rate of all filtration trials across sites. \\label{FR_Site_boxplot}"}
...
## Stack Exchange -TEX help suggestion
rm(list=ls()) 
library(xtable) 
data(tli) 
xtable(tli[1:10, ])

### simply example to figure out xtable error

# trying to download olderr version of data.table. Won't install newest version on Mac
require(remotes)
install_version("data.table", version = "1.12.6", repos = "http://cran.us.r-project.org")
...
```{r Dr Nichols code, include=FALSE}
...
Combined filtration trials had significantly higher HCRs (habitat clearance rate) than negative controls (two sample t(`r t_test_df`) = `r round(filter_control_ttest$statistic, digits = 2)`, p = `r round(filter_control_ttest$p.value, digits = 2)`).

```{r FR_ChlRmd_Site_posFilter_boxplot, echo=FALSE, include=F, fig.keep = 'all', dev = 'pdf',fig.show="hold", fig.align="default", fig.asp=0.618, fig.width=4.3, out.width="50%", message= F, fig.cap="The habitat level filtration rates and percent chlorophyll removed of filtration trials (positive values only) across sites. \\label{FR_ChlRmd_Site_posFilter_boxplot}"}
...
FRPos_TPM_graph <- ggplot((Filtration_pos_data), aes(Avg_TPM_mg_L, L_hr_m2 )) +
...
FRPos_TPM_lm <- lm(data = Filtration_pos_data, L_hr_m2 ~ Avg_TPM_mg_L)
#summary(FR_TPM_lm)
...
```{r NPD_FRPos_TPM_graph, echo=FALSE, eval=F, fig.keep = 'all', dev = 'pdf', fig.show="hold", fig.show = "hold", fig.align="default", fig.asp=0.618, fig.width=4.3, out.width="50%", message= F, fig.cap="The positive habitat level filtration rates in response to seston total particulate matter (TPM) at the Deanza site in Newport Bay, California. \\label{NPD_FRPos_TPM_graph}"}
...
TPM_OC <- ggplot(data = Avg_TPM_summary_table, aes(x = Avg_TPM_mg_L, y = Avg_OC_Ratio)) +
...
TPM_box <- ggplot((Avg_TPM_summary_table), aes(Site, Avg_TPM_mg_L)) +
  geom_boxplot(color = Site_colors) +
  theme_classic() +
...
OC_logTPM_all_LM <- lm(data = Avg_TPM_summary_table, Avg_OC_Ratio ~ log(Avg_TPM_mg_L))
TPM_newrange <- data.frame(Avg_TPM_mg_L = seq(from = 0, to = 0.075, by = 0.005))
...
OC_logTPM_graph <- ggplot(Avg_TPM_summary_table, aes(Avg_TPM_mg_L, Avg_OC_Ratio)) +
  geom_line(data = pred_TPMlog_df, aes(x = Avg_TPM_mg_L, y = Avg_OC_Ratio)) +
  geom_point(aes(color = Site)) +
  scale_color_manual(values = Site_colors) + 
  theme_classic() +
  labs(x = TPM_Label,
...
OC_TPM_all_LM <- lm(data = Avg_TPM_summary_table, Avg_OC_Ratio ~ Avg_TPM_mg_L)
...
logOC_logTPM_all_LM <- lm(data = Avg_TPM_summary_table, log(Avg_OC_Ratio) ~ log(Avg_TPM_mg_L))
...
```{r Bivalve_Den_bargraph, echo=FALSE, fig.align="center", fig.asp=0.618, fig.width=6, warning= F, message= F, fig.keep='all', dev='pdf', out.width="70%", fig.cap="The bivlaves community density for each study site. San Rafel data was collected November 2017 by the Zabin lab, Morro Bay was estimated by Morro Bay Oyster Company, and Shellmaker and Deanza were surveyed in May 2018 by the Zacherl lab.  \\label{Bivalve_Den_Biomass}"}
