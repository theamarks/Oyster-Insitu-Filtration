...
Filtration trials across sites were not distributed equally (Table 1), Deanza had more than twice the amount filtration trials (*N* = 9) as San Rafael (*N* = 4), Morro Bay (*N* = 4), and Shellmaker (*N* = 4), and each site had a single control.
...
A power analysis of this one-way ANOVA determined that effect size between site and Chl $\alpha$ removal is very small (*ES* = `r format(round(Chlrmd_site_power$f, 2), nsmall = 2)`) [@Sawilowsky2009a] with little statistical power (`r round(Chlrmd_site_power$power,2)`).
...
A statistical power analysis determined that the effect size between filtration and control trial HCR was very large (*ES* = `r round(test1_power$d, 2)`), per the criteria in @Sawilowsky2009a, and there was little power (`r round(test1_power$power,2)`) to detect a statistical signal.
...
Despite non-significant results between control and filtration trials, statistical power analysis indicate that my limited sample size is inhibiting statistical inference of a very large effect size of filtration trial HCRs compared to control HCRs.
