...
```{r Site WQ ANOVA, fig.width=7, fig.asp=1.3, fig.cap="Box plots of ambient (upstream) water quality variables from filtration and control trials. Letters indicate post-hoc Tukey results among sites. Trials were conducted from February 2018 to June 2019 at San Rafael, CA (restored reefs); Morro Bay, CA (Morro Bay Oyster Companyaqu (aqculture); and Newport Bay, CA (Shellmaker and Deanza, restored beds). \\label{Site_WQ_boxplot}" }
#########################
###### Chlorphyll
#hist(site_data_outrmd$Chl_ug_L_Up)
site_chl_aov <- aov(Chl_ug_L_Up ~ Site, data = site_data_outrmd)
...
Chl_box <- ggplot(site_data_outrmd, aes(Site, Chl_ug_L_Up)) +
  ...
  theme_classic() +
  labs(x = 'Site',
       y = expression(paste("Upstream Chlorophyll ",alpha, " ", mu,"g/L"))) +
  annotate("text", "San Rafael", 
           max((site_data_outrmd %>% filter(Site == "San Rafael"))$Chl_ug_L_Up) + 0.5, label = "a") +
  annotate("text", "Morro Bay", 
           max((site_data_outrmd %>% filter(Site == "Morro Bay"))$Chl_ug_L_Up) + 0.5, label = "ab") +
  annotate("text", "Shellmaker", 
           max((site_data_outrmd %>% filter(Site == "Shellmaker"))$Chl_ug_L_Up) + 0.5, label = "b") +
  annotate("text", "Deanza", 
           max((site_data_outrmd %>% filter(Site == "Deanza"))$Chl_ug_L_Up) + 0.5, label = "ab") 

#########################
###### Temperature 
#hist(site_data_outrmd$Temp_C_Up)
site_temp_aov <- aov(Temp_C_Up ~ Site, data = site_data_outrmd)
...
Temp_box <- ggplot(site_data_outrmd, aes(Site, Temp_C_Up)) +
  geom_boxplot(color = Site_colors) + 
  geom_dotplot(binaxis = "y", stackdir = "center", aes(fill = Site),
               alpha = 0.5, dotsize = 0.65, show.legend = FALSE) +
  scale_fill_manual(values = Site_colors) +
  theme_classic() +
  labs(x = 'Site',
       y = expression(paste("Upstream Temperature ",degree, "C"))) +
  annotate("text", "San Rafael", 
           max((site_data_outrmd %>% filter(Site == "San Rafael"))$Temp_C_Up) + 0.5, label = "a") +
  annotate("text", "Morro Bay", 
           max((site_data_outrmd %>% filter(Site == "Morro Bay"))$Temp_C_Up) + 0.5, label = "a") +
  annotate("text", "Shellmaker", 
           max((site_data_outrmd %>% filter(Site == "Shellmaker"))$Temp_C_Up) + 0.5, label = "a") +
  annotate("text", "Deanza", 
           max((site_data_outrmd %>% filter(Site == "Deanza"))$Temp_C_Up) + 0.5, label = "a") 

#########################
####### Salinity
#hist(site_data_outrmd$Sal_ppt_Up)
site_sal_aov <- aov(Sal_ppt_Up ~ Site, data = site_data_outrmd)
...
Sal_box <- ggplot(site_data_outrmd, aes(Site, Sal_ppt_Up)) +
  geom_boxplot(color = Site_colors) + 
  geom_dotplot(binaxis = "y", stackdir = "center", aes(fill = Site),
               alpha = 0.5, dotsize = 0.65, show.legend = FALSE) +
  scale_fill_manual(values = Site_colors) +
  theme_classic() +
  labs(x = 'Site',
       y = "Upstream Salinity ppt") +
  annotate("text", "San Rafael", 
           max((site_data_outrmd %>% filter(Site == "San Rafael"))$Sal_ppt_Up) + 0.5, label = "a") +
  annotate("text", "Morro Bay", 
           max((site_data_outrmd %>% filter(Site == "Morro Bay"))$Sal_ppt_Up) + 0.5, label = "b") +
  annotate("text", "Shellmaker", 
           max((site_data_outrmd %>% filter(Site == "Shellmaker"))$Sal_ppt_Up) + 0.5, label = "b") +
  annotate("text", "Deanza", 
           max((site_data_outrmd %>% filter(Site == "Deanza"))$Sal_ppt_Up) + 0.5, label = "b") 

###########################
######### Turbidity
#hist(site_data_outrmd$Turbidity_NTU_Up)
site_turb_aov <- aov(Turbidity_NTU_Up ~ Site, data = site_data_outrmd)
...
Turb_box <- ggplot(site_data_outrmd, aes(Site, Turbidity_NTU_Up)) +
  geom_boxplot(color = Site_colors) + 
  geom_dotplot(binaxis = "y", stackdir = "center", aes(fill = Site),
               alpha = 0.5, dotsize = 0.65, show.legend = FALSE) +
  scale_fill_manual(values = Site_colors) +
  theme_classic() +
  labs(x = 'Site',
       y = "Upstream Turbidity NTU") +
  annotate("text", "San Rafael", 
           max((site_data_outrmd %>% filter(Site == "San Rafael"))$Turbidity_NTU_Up) + 2, 
           label = "a") +
  annotate("text", "Morro Bay", 
           max((site_data_outrmd %>% filter(Site == "Morro Bay"))$Turbidity_NTU_Up) + 2, 
           label = "b") +
  annotate("text", "Shellmaker", 
           max((site_data_outrmd %>% filter(Site == "Shellmaker"))$Turbidity_NTU_Up) + 2, 
           label = "c") +
  annotate("text", "Deanza", 
           max((site_data_outrmd %>% filter(Site == "Deanza"))$Turbidity_NTU_Up) + 2, 
           label = "c") 

########################
####### TPM
#hist(site_data_outrmd$Avg_TPM_mg_L)
site_TPM_aov <- aov(Avg_TPM_mg_L ~ Site, data = site_data_outrmd)
...
Tpm_box <- ggplot(site_data_outrmd, aes(Site, Avg_TPM_mg_L)) +
  geom_boxplot(color = Site_colors) + 
  geom_dotplot(binaxis = "y", stackdir = "center", aes(fill = Site),
               alpha = 0.5, dotsize = 0.65, show.legend = FALSE) +
  scale_fill_manual(values = Site_colors) +
  theme_classic() +
  labs(x = 'Site',
       y = expression(paste("Upstream Total Particulate Matter ",mu, "g/L"))) +
  annotate("text", "San Rafael", 
           max((site_data_outrmd %>% 
                  filter(Site == "San Rafael"))$Avg_TPM_mg_L) + 5, label = "a") +
  annotate("text", "Morro Bay", 
           max((site_data_outrmd %>% 
                  na.omit() %>% filter(Site == "Morro Bay"))$Avg_TPM_mg_L) + 5, label = "b") +
  annotate("text", "Shellmaker", 
           max((site_data_outrmd %>% 
                  filter(Site == "Shellmaker"))$Avg_TPM_mg_L) + 5, label = "b") +
  annotate("text", "Deanza", 
           max((site_data_outrmd %>% 
                  na.omit() %>% filter(Site == "Deanza"))$Avg_TPM_mg_L) + 5, label = "b") 

#####################
####### OC
#hist(site_data_outrmd$Avg_OC_Ratio)
site_OC_aov <- aov(Avg_OC_Ratio ~ Site, data = site_data_outrmd)
...
OC_box <- ggplot(site_data_outrmd, aes(Site, Avg_OC_Ratio)) +
  geom_boxplot(color = Site_colors) + 
  geom_dotplot(binaxis = "y", stackdir = "center", aes(fill = Site),
               alpha = 0.5, dotsize = 0.65, show.legend = FALSE) +
  scale_fill_manual(values = Site_colors) +
  theme_classic() +
  labs(x = 'Site',
       y = "Upstream Organic Content Ratio ") +
  annotate("text", "San Rafael", 
           max((site_data_outrmd %>% 
                  filter(Site == "San Rafael"))$Avg_OC_Ratio) + 0.025, label = "a") +
  annotate("text", "Morro Bay", 
           max((site_data_outrmd %>% 
                  na.omit() %>% filter(Site == "Morro Bay"))$Avg_OC_Ratio) + 0.025, label = "b") +
  annotate("text", "Shellmaker", 
           max((site_data_outrmd %>% 
                  filter(Site == "Shellmaker"))$Avg_OC_Ratio) + 0.025, label = "ab") +
  annotate("text", "Deanza", 
           max((site_data_outrmd %>% 
                  na.omit() %>% filter(Site == "Deanza"))$Avg_OC_Ratio) + 0.025, label = "b") 

grid.arrange(Chl_box, Temp_box, Sal_box, Turb_box, Tpm_box, OC_box, nrow=3)
```
...
A single filtration trial at Deanza (2019-4-17; Table 1) was removed from the analysis because the mean upstream Chl $\alpha$ (*M* = `r round(mean(NPD_2019_4_17$Chl_ug_L_Up),1)`, *SD* = `r round(sd(NPD_2019_4_17$Chl_ug_L_Up),2)`) was within the detection limit of the sensor (± 0.1 $\mu$/L). 
Filtration trials across sites were not distributed equally (Table 1), Deanza had more than twice the amount filtratrion trials (*N* = 9) as San Rafael (*N* = 4), Morro Bay (*N* = 4), and Shellmaker (*N* = 4), and each site had a single control.
Ambient water quality varied within and among sites (Figure \ref{Site_WQ_boxplot}). 
...
The mean percent Chl $\alpha$ removal at the San Rafael site was `r round(mean(SR_filter_sum$pcnt_Chl_rmvd),1)`\% (*N* = `r nrow(SR_filter_sum)`, *SD* = `r round(sd(SR_filter_sum$pcnt_Chl_rmvd),2)`) (Figure \ref{ChlRmd_Site_boxplot}) and was `r round(SR_NC_sum$pcnt_Chl_rmvd,1)`\% in the single control trial. 
...
Mean Shellmaker Chl $\alpha$ removal was `r round(mean(NPSM_filter_sum$pcnt_Chl_rmvd),1)` \% (*N* = `r nrow(NPSM_filter_sum)`, *SD* = `r round(sd(NPSM_filter_sum$pcnt_Chl_rmvd),1)`) (Figure \ref{ChlRmd_Site_boxplot}), and its control trial was `r round(mean(NPSM_NC_sum$pcnt_Chl_rmvd),1)` \%.


```{r ChlRmd_Site_boxplot, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align= "center", fig.asp=0.618, out.width="70%", message= F, fig.cap="Box plots of percent chlorophyll $\\alpha$ removal (Chl\\textsubscript{up} - Chl\\textsubscript{down} / Chl\\textsubscript{up} * 100) during filtration trials, control trials are not displayed. Each data point is an the average value from a single filtration trial. Letters are site differences from a one-way ANOVA with Tukey's HSD, and diamonds are site means. Filtration trials were conducted between February 2018 to June 2019 at San Rafael, CA (restored reefs); Morro Bay, CA (Morro Bay Oyster Company aquaculture); and Newport Bay, CA (Shellmaker and Deanza, restored beds). Each data point is the average value from a single filtration trial. \\label{ChlRmd_Site_boxplot}"}
...
Chl_Rmd_box <- ggplot(Filter_only_outRmd_data, aes(Site, pcnt_Chl_rmvd)) +
  ...
  labs(x = 'Site',
       y = Prt_Chl_Label) +
   annotate("text", "San Rafael", 
           max((Filter_only_outRmd_data %>% 
                  filter(Site == "San Rafael"))$pcnt_Chl_rmvd) + 4, label = "a") +
   annotate("text", "Morro Bay", 
           max((Filter_only_outRmd_data %>% 
                  filter(Site == "Morro Bay"))$pcnt_Chl_rmvd) + 4, label = "a") +
   annotate("text", "Shellmaker", 
           max((Filter_only_outRmd_data %>% 
                  filter(Site == "Shellmaker"))$pcnt_Chl_rmvd) + 4, label = "a") +
   annotate("text", "Deanza", 
           max((Filter_only_outRmd_data %>% 
                  filter(Site == "Deanza"))$pcnt_Chl_rmvd) + 4, label = "a")
plot(Chl_Rmd_box)
...
dim(Analysis_data[,-c(1,2,3)]) # dimensions of data with labels removed (data only) 28 x 12 = 336 data cells
...
# remove extreme values (NPSM 2020_6_9, NPSM 2020_5_22, NPD 2020_4_17)
...
### Alternative value removal
# remove one single trial (NPD_2020_4_17) avg Chlup at sensor detection limit (+- 0.1 ug/L)
test1 = test %>% 
  dplyr::filter(round(Chl.up,2) > 0.1 | round(Chl.up,2) < -0.1)
# check distribution of Control trials
check_control_dist <- test %>% 
  filter(Experiment %in% c("Neg Control 1"))
hist(check_control_dist$Clearance)
...
check_filter_dist <- test %>% 
  filter(Experiment %in% c("Filtration"))
hist(check_filter_dist$Clearance)
...
```{r HCR_Quantile_Regression, eval=FALSE, include=FALSE}
...
library(quantreg)
# 25TH QUANTILE
QR_HCR_25 <- rq(L_hr_m2 ~ Temp_C_Up + Sal_ppt_Up + Turbidity_NTU_Up + Avg_TPM_mg_L + Avg_OC_Ratio + Site, 
                tau = 0.25, data = Filter_only_outRmd_data)
summary(QR_HCR_25)
# 50TH QUANTILE
QR_HCR_50 <- rq(L_hr_m2 ~ Temp_C_Up + Sal_ppt_Up + Turbidity_NTU_Up + Avg_TPM_mg_L + Avg_OC_Ratio + Site, 
                tau = 0.5, data = Filter_only_outRmd_data)
summary(QR_HCR_50)
# 75TH QUANTILE
QR_HCR_75 <- rq(L_hr_m2 ~ Temp_C_Up + Sal_ppt_Up + Turbidity_NTU_Up + Avg_TPM_mg_L + Avg_OC_Ratio + Site,
                data = Filter_only_outRmd_data)
summary(QR_HCR_75)
# 90TH QUANTILE
QR_HCR_90 <- rq(L_hr_m2 ~ Temp_C_Up + Sal_ppt_Up + Turbidity_NTU_Up + Avg_TPM_mg_L + Avg_OC_Ratio + Site,
                tau = 0.90, data = Filter_only_outRmd_data)
summary(QR_HCR_90)

# ANOVA to see if models are different
anova(QR_HCR_25,QR_HCR_75)
anova(QR_HCR_25,QR_HCR_90)

# Plot all
qr_all <- rq(L_hr_m2 ~ Temp_C_Up + Sal_ppt_Up + Turbidity_NTU_Up + Avg_TPM_mg_L + Avg_OC_Ratio + Site,
...
par(mfrow=c(18,2))
plot(qr_all)
plot.rqs(summary(qr_all)) # why aren't two red dashed CI displaying?

# ggplot Quantile regression
q3 <- c(0.25, 0.5, 0.75)
q4 <- c(0.25, 0.5, 0.75, 0.95)
q1 <- seq(0.05, 0.95, by = 0.05)
...
qrplot_temp <- ggplot(Filter_only_outRmd_data, aes(Temp_C_Up, L_hr_m2)) +
  ...
  geom_quantile(quantiles = q4, color = 'grey50')
qrplot_temp 
# Salinity plot
...
grid.arrange(qrplot_temp, qrplot_sal, qrplot_turb, qrplot_tpm, qrplot_oc, nrow = 3)
```

## Oyster Habitat vs. Control 

Filtration trials had significantly higher habitat clearance rates (HCR) than negative controls (two-sample *t*(`r t_test_df`) = `r round(filter_control_ttest$statistic, digits = 2)`, *p* = `r round(filter_control_ttest$p.value, digits = 2)`).
The random forest model that included all filtration and control trials (*R*^2^ = `r round(r.sq,2)`) indicated that temperature had the highest relative importance to the model (`r round(Temp_rel_import, 2)`\%), followed by OC (`r round(OC_rel_import, 1)`\%), turbidity (`r round(Turb_rel_import, 1)`\%), TPM (`r round(TPM_rel_import, 1)`\%), salinity (`r round(Sal_rel_import, 1)`\%), and site (`r round(Site_rel_import, 1)`\%).
Filtration and control trials adjusted by the random forest model were not significantly different (two-sample *t*(`r t_test_adj_df`) = `r round(t_test_adj$statistic, digits = 2)`, *p* = `r round(t_test_adj$p.value, digits = 3)`); however, my limited sample size and variable clearance rates are expected to inhibit statistical signal detection.

## Habitat Clearance Rates 

HCR at Ran Rafael averaged `r round(mean(SR_filter_sum$L_hr_m2),1)` L hr^-1^ m^-2^ (*N* = `r nrow(SR_filter_sum)`, *SD* = `r round(sd(SR_filter_sum$L_hr_m2),1)`) (Figure \ref{FR_Site_boxplot}), 
at Morro Bay averaged `r round(mean(MB_filter_sum$L_hr_m2),1)` L hr^-1^ m^-2^ (*N* = to `r nrow(MB_filter_sum)`, *SD* = `r round(sd(MB_filter_sum$L_hr_m2),1)`), 
at Shellmaker averaged `r round(mean(NPSM_filter_sum$L_hr_m2),1)` L hr^-1^ m^-2^ (*N* = `r nrow(NPSM_filter_sum)`, *SD* = `r round(sd(NPSM_filter_sum$L_hr_m2),1)`), 
and at Deanza averaged `r round(mean(NPD_filter_sum$L_hr_m2),1)` L hr^-1^ m^-2^ (*N* = `r nrow(NPD_filter_sum)`, *SD* = `r round(sd(NPD_filter_sum$L_hr_m2),1)`).
Control trials measured `r round(SR_NC_sum$L_hr_m2,1)`, and `r round(MB_NC_sum$L_hr_m2,1)` L hr^-1^ m^2^ for San Rafael, and Morro Bay respectively, Shellmaker's control trial was removed from the analysis as an extreme value (Figure \ref{Corr_to_signal_95CI_graph}).
Newport Deanza's three consecutive control trials averaged `r round(mean(NPD_NC_sum$L_hr_m2))` L hr^-1^ m^-2^ (*N* = `r nrow(NPD_NC_sum)`, *SD* = `r round(sd(NPD_NC_sum$L_hr_m2),1)`).
Site did not significantly affect HCR as determined by a one-way ANOVA at a *p* < .05 level (*F*(3,16) = 0.29, *p* = 0.83) (Table 2).

```{r FR_Site_boxplot, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align= "center", fig.asp=0.618, out.width="70%", message= F, fig.cap="Box plots of habitat clearance rates (HCR) during filtration trials, control trials are not displayed. Each data point is an the average value from a single filtration trial. Letters are post-hoc Tukey honestly significant difference results, and diamonds are site means. Filtration trials were conducted between February 2018 to June 2019 at San Rafael, CA (restored \\textit{O. lurida} reefs); Morro Bay, CA (Morro Bay Oyster Company \\textit{C. gigas} aquaculture); and Newport Bay, CA (Shellmaker and Deanza, restored beds) Extreme values excluded from analysis are not shown. \\label{FR_Site_boxplot}"}
...
FR_box <- ggplot((Filter_only_outRmd_data), aes(Site, L_hr_m2)) +
  ...
  geom_dotplot(binaxis = "y", stackdir = "center", aes(fill = Site),
               alpha = 0.5, dotsize = 0.65, show.legend = FALSE) +
   geom_point(data = FR_Site, aes(Site, avg_L_hr_m2),
             size = 2, shape = 18) +
  scale_fill_manual(values = Site_colors) +
  theme_classic() +
  #stat_summary(fun.data = n_mean_stats, geom = "text") +
  ...
  annotate("text", "San Rafael", 
           max((Filter_only_outRmd_data %>% 
                  filter(Site == "San Rafael"))$L_hr_m2) + 200, label = "a") +
  annotate("text", "Morro Bay", 
           max((Filter_only_outRmd_data %>% 
                  filter(Site == "Morro Bay"))$L_hr_m2) + 200, label = "a") +
  annotate("text", "Shellmaker", 
           max((Filter_only_outRmd_data %>% 
                  filter(Site == "Shellmaker"))$L_hr_m2) + 200, label = "a") +
  annotate("text", "Deanza", 
           max((Filter_only_outRmd_data %>% 
                  filter(Site == "Deanza"))$L_hr_m2) + 200, label = "a")
plot(FR_box) 

# Example data for DZ to try ANOVA by site HCR
#fwrite(Filter_only_outRmd_data, "Filter_data_HCR_ForDZ.csv")
...
```{r Site Summary Table, eval=FALSE, include=FALSE}

#### Redundant with Site WQ graphs
Site_Sum <- Final_table %>% 
  filter(!Date == "2019-04-17") %>% 
  dplyr::group_by(Site) %>% 
  dplyr::summarize(Chl = paste0(round(mean(na.omit(Chl_ug_L_Up)),1),
                                ' (SD ', 
                                 round(sd(na.omit(Chl_ug_L_Up)),2), 
                                 ')'),
                  Temp = paste0(round(mean(na.omit(Temp_C_Up)),2),
                                 ' (SD ', 
                                 round(sd(na.omit(Temp_C_Up)),2), 
                                 ')'),
                   Sal = paste0(round(mean(na.omit(Sal_ppt_Up)),1),
                                ' (SD ', 
                                 round(sd(na.omit(Sal_ppt_Up)),1), 
                                 ')'),
                   Turb = paste0(round(mean(na.omit(Turbidity_NTU_Up)),1),
                                 ' (SD ', 
                                 round(sd(na.omit(Turbidity_NTU_Up)),1), 
                                 ')'),
                   TPM = paste0(round(mean(na.omit(Avg_TPM_mg_L)),2), 
                                ' (SD ', 
                                 round(sd(na.omit(Avg_TPM_mg_L)),2), 
                                 ')'),
                   OC = paste0(round(mean(na.omit(Avg_OC_Ratio)),2),
                               ' (SD ', 
                                 round(sd(na.omit(Avg_OC_Ratio)),2), 
                                 ')'))

#site_sum_table <- cbind(Site_Sum, 
 #                       tibble(Lat = c(37.964179, 35.334707, 33.622097, 33.620291),
  #                      Long = c(-122.487217, -120.844000, -117.892399, -117.897692)#,
                        #Months = c("July, Nov", "July, May, June", "May, June", "Oct, April, May, June")
   #                     ))

Site_Sum %<>% 
  dplyr::select(Site, Chl, Temp, Sal, Turb, TPM, OC)
  
site_sum_xtable <- xtable(Site_Sum, 
                         caption = "Summary of Environmental Conditions at Each Study Site.",
                         label = "tb:site",
                         align = "llllllll",
                         type = "latex")

print.xtable(site_sum_xtable, 
      booktabs=TRUE, 
      caption.placement="top", 
      NA.string = NA, 
      sanitize.colnames.function=function(x){x},
...
```{r HCR_Site_ANOVA, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, results='asis'}

## ANOVA Data
FR_Site_alldata <- Filter_only_outRmd_data %>% 
  dplyr::select(Date, Site, L_hr_m2)

## Check Normality assumption - looks ok..
# hist(FR_Site_alldata$L_hr_m2)

## ANOVA HCR ~ Site
model <- lm(L_hr_m2 ~ Site, data = FR_Site_alldata)
aov.model <- anova(model)
#summary(aov.model)

## Create ANOVA results table in LaTeX
aov.xt <- xtable(aov.model,
                       caption = 'Table 2: One-way ANOVA of the effect of site on habitat clearance rate (HCR L hr\\textsuperscript{-1} m\\textsuperscript{-2})')
colnames(aov.xt) <- c("df", "Sum Sq", "Mean Sq", "\\textit{F} value", "\\textit{p} value")
print(aov.xt,
      type="latex",
      booktabs = T,
      caption.placement = 'top',
...
The random forest model using only filtration trials (*R*^2^ = `r round(r.sq.filter, 2)`) indicated that temperature (`r round(Temp_rel_import_HCR,1)`\%) had the highest relative importance to the model, followed by turbidity (`r round(Turb_rel_import_HCR,1)`\%), TPM (`r round(TPM_rel_import_HCR,1)`\%), OC (`r round(OC_rel_import_HCR,1)`\%), site (`r round(Site_rel_import_HCR,1)`\%), and salinity (`r round(Sal_rel_import_HCR,1)`\%).
...
```{r TPM_OC_graphs, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align="center", fig.asp=0.618, fig.width=6, out.width="70%", warning=F, message= F, fig.cap="The relationship between seston total particulate matter (TPM) and seston organic content (OC) across all sites and all trials. Two field days are not included because of missing TPM data. \\label{TPM_OC_model}"}
############ Linear Model
...
Total Particulate Matter (TPM) is a significant negative predictor of seston OC when sites are combined (ln(y) = `r round(coef(logOC_logTPM_LM_sum)[1,1],digits=2)` `r round(coef(logOC_logTPM_LM_sum)[2,1],digits=3)` ln(x), *R*^2^ = `r  round(logOC_logTPM_LM_sum$r.squared,digits=2)`, *p* = `r round(coef(logOC_logTPM_LM_sum)[2,4],digits = 3)`) (Figure \ref{TPM_OC_model}). 
This model includes all field days (filtration and controls) because water samples used to determine TPM and OC were collect the same way regardless of the trial. 
Bays have distinct TPM signatures that do not overlap (Figure \ref{TPM_OC_model}).
Northern San Francisco Bay (San Rafael) TPM averaged `r round(mean(SR_TPM_OC$Avg_TPM_mg_L),2)` mg/L (*N* = `r nrow(SR_TPM_OC)`, *SD* = `r round(sd(SR_TPM_OC$Avg_TPM_mg_L),2)`), and Morro Bay TPM averaged `r round(mean(MB_TPM_OC$Avg_TPM_mg_L),2)` mg/L (*N* = `r nrow(MB_TPM_OC)`, *SD* = `r round(sd(MB_TPM_OC$Avg_TPM_mg_L),2)`).
...
Newport Bay (Deanza and Shellmaker) OC averaged `r round(mean(NP_combo_TPM_OC$Avg_OC_Ratio),3)` (*N* = `r nrow(NP_combo_TPM_OC)`, *SD* = `r round(sd(NP_combo_TPM_OC$Avg_OC_Ratio),2)`).
...
In November 2017 the estimated bivalve density at San Rafael was `r SR_Den` individuals/m^2^, all bivalves were *Ostrea lurida* (Figure \ref{Bivalve_Den_Biomass}).
Other bivalves were noted, but rare, and were not detected in sample bags (C. Zabin, unpublished data). 
Morro Bay has an estimated `r MB_Den` *Crassostrea gigas* individuals/m^2^ (Morro Bay Oyster Company), personal field observations confirm the lack of bivalve fouling on the aquaculture lines.
Shellmaker has an estimated `r NPSM_Den` individuals/m^2^, composed of *Adula diegensis* (`r NPSM_species[NPSM_species$Species=="Adula diegensis",2]` individuals/m^2^), *Musculista senhousia* (`r NPSM_species[NPSM_species$Species=="Musculista senhousia",2]` individuals/m^2^), *O. lurida* (`r NPSM_species[NPSM_species$Species=="Ostrea lurida",2]` individuals/m^2^), *Mytilus galloprovincialis* (`r NPSM_species[NPSM_species$Species=="Mytilus galloprovincialis",2]` individuals/m^2^), *Geukensia demissa* (`r NPSM_species[NPSM_species$Species=="Geukensia demissa",2]` individuals/m^2^), and *Argopecten ventricosa* (`r NPSM_species[NPSM_species$Species=="Argopecten ventricosa",2]` individuals/m^2^) (Figure \ref{Bivalve_Den_Biomass}.
Deanza has an estimated `r NPD_Den` individuals/m^2^, composed of *M. senhousia* (`r NPD_species[NPD_species$Species=="Musculista senhousia",2]` individuals/m^2^), *A. diegensis* (`r NPD_species[NPD_species$Species=="Adula diegensis",2]` individuals/m^2^), *O. lurida* (`r NPD_species[NPD_species$Species=="Ostrea lurida",2]` individuals/m^2^), *M. galloprovincialis* (`r NPD_species[NPD_species$Species=="Mytilus galloprovincialis",2]` individuals/m^2^) (Figure \ref{Bivalve_Den_Biomass}.

```{r Bivalve_Den_bargraph, echo=FALSE, fig.align="center", fig.asp=0.618, fig.width=6, warning= F, message= F, fig.keep='all', dev='pdf', out.width="70%", fig.cap="Bivalve community density and composition for each study site. San Rafael data was collected November 2017 by the Zabin lab at SERC, Morro Bay was estimated by Morro Bay Oyster Company, and Shellmaker and Deanza were surveyed in May 2018 by the Zacherl lab at CSUF.  \\label{Bivalve_Den_Biomass}"}
...
```{r Oly_DTW_SH_outrmd_Graphs, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align="center", warning=F, message=F, fig.asp=0.618, fig.width=6, out.width="70%", fig.cap="The relationship between \\textit{O. lurida} shell length and dry tissue weight (DTW) in Newport Bay, California at the Deanza restoration site. \\label{NPD_Oly_DTW_SH_Trans}"}
...
```{r Research_Comparison, include=FALSE}
FR_Site # Average HCR by site
NPD_Oly_HCR = FR_Site[[4,2]] # NPD average HCR l hr-1 m-2
NPD_Oly_Bio = NPD_species_DTW[[4,2]] # NPD Oly DTW (g m-2)
Gray_Oly_dry_CR = 0.78 # (l hr-1 g-1) Gray dry season Oly avg CR

NPD_CR_est = NPD_Oly_Bio*Gray_Oly_dry_CR
NPD_CR_est # adjust my NPD HCR by Oly biomass to compare to Gray CR


Avg_WQ <- Filter_only_outRmd_data %>% 
  group_by(Site) %>% 
  na.omit() %>% 
  summarize(avg_OC = mean(Avg_OC_Ratio),
            avg_Temp = mean(Temp_C_Up))

Gray_Oly_CR <- (-0.96) + (2.49*Avg_WQ[[4,2]]) + (0.06*Avg_WQ[[4,3]])
NPD_HCR_est = Gray_Oly_CR * NPD_Oly_Bio
