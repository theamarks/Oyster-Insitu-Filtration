...
```{r logChlRmdPos_logTPM_lm_diag, cache=TRUE, include=FALSE}
par(mfrow=c(2,2))
...
```{r ChlRmdPos_TPM2_lm_diag, cache=TRUE, include=FALSE}
par(mfrow=c(2,2))
...
```{r TPM_OC_graphs, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align="default", fig.asp=0.618, fig.show="hold", fig.width=6, out.width="50%", fig.cap="The positive habitat level filtration rates in response to seston total particulate matter (TPM). \\label{figurelabel}"}
...
### ***All TMP ~ OC***
```{r fig.height=6, fig.width=6, cache=TRUE, include=FALSE}
OC_TPM_all_LM <- lm(data = Avg_TPM_summary_table, Avg_OC_Ratio ~ Avg_TPM)
ols_regress(OC_TPM_all_LM)
```

### ***All logTMP ~ OC***
```{r OC_logTPM_all_LM, fig.height=6, fig.width=6, cache=TRUE, include=FALSE}
OC_logTPM_all_LM <- lm(data = Avg_TPM_summary_table, Avg_OC_Ratio ~ log(Avg_TPM))
ols_regress(OC_logTPM_all_LM)
```

### ***Y ~ logX Diagnostics***
```{r fig.height=6, fig.width=8, cache=TRUE, include=FALSE}
par(mfrow=c(2,2))
...
### ***All logTMP ~ logOC***
```{r logOC_logTPM_all_LM, fig.height=6, fig.width=6, cache=TRUE, include=FALSE}
logOC_logTPM_all_LM <- lm(data = Avg_TPM_summary_table, log(Avg_OC_Ratio) ~ log(Avg_TPM))
ols_regress(logOC_logTPM_all_LM)
```

### ***TPM vs OC by sites***
```{r TPM_OC_Sites, fig.height=10, fig.width=12, cache=TRUE, include=FALSE}
...
```{r NPD_Filter_Temp, fig.height=10, fig.width=12, cache=TRUE, include=FALSE}
...
### ***Filtration - Salinity***
```{r Filter_Salinity, fig.height=5, fig.width=14, cache=TRUE, include=FALSE}
...
```{r LogChlRmdPos_LogSalinity_graph, fig.height=12, fig.width=12, cache=TRUE, include=FALSE}
...
```{r NPD_Filter_Salinity, fig.height=5, fig.width=12, cache=TRUE, include=FALSE}
...
```{r Filter_Chl, fig.height=15, fig.width=14, cache=TRUE, include=FALSE}
...
```{r NPD_Filter_Chl, fig.height=5, fig.width=12, cache=TRUE, include=FALSE}
...
```{r Filter_Turbidity, fig.height=15, fig.width=14, cache=TRUE, include=FALSE}
...
```{r NPD_Filter_Turbidity, fig.height=5, fig.width=12, cache=TRUE, include=FALSE}
...
```{r Oly_DTW_SH_outLier_removed, fig.height=5, fig.width=6, cache=TRUE, include=FALSE}
...
### ***Log Transform DTW***
```{r fig.height=10, fig.width=12, cache=TRUE, include=FALSE}
...
grid.arrange(Oly_SH_logxDTW, Oly_shell_DTW_logline, 
...
Oly_TWW_DTW_rm_out <- ggplot(NPD_oly_TWW, aes(total_wet_weight_g, tissue_dry_weight_g)) +
  ...
  labs(x = 'Total wet weight (g)',
       ...
       subtitle = 'Gray & Langdon 2017 relationship in blue DTW = 0.0044TWW − 0.043') +
  ggpmisc::stat_poly_eq(aes(label = paste("atop(",..eq.label.., ",",..adj.rr.label.., ")", sep ="")),
               formula = x_y_formula,
               parse = TRUE,
               label.x = "left",
...
grid.arrange(Oly_TWW_DTW_rm_out, Oly_logTWW_DTW_graph,
...
### ***Bivalve Density***
```{r Bivalve_Density, fig.height=6, fig.width=8, cache=TRUE, include=FALSE}
...
```{r Bar_Graphs_up_vs_down_grid.arrange, cache=TRUE, include=FALSE}
