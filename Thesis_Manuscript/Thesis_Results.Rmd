...
```{r Correction_to_Signal_95CI_graph, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.show = "hold", fig.align= 'center', out.width="70%", fig.asp=0.618, fig.width=6, message=FALSE, warning=FALSE,  fig.cap= "The relationship between side-by-side chlorophyll difference (correction) and experiment trial chlorophyll difference (signal). The solid black line is a 1-1 relationship, the dashed red lines are 95\\% confience intervals of the signal-to-correction relationship. Identified extreme values with 95\\% confience interval were removed from analysis and model. Confience intervals were recalculated with 98\\% intervals to identify addition extreme values. \\label{Corr_to_signal_95CI_graph}"}

#The relationship between side-by-side chlorophyll corrections and experiment trial chlorophyll difference (signal). Solid black line is the linear model, dashed red lines are confience intervals. A) Identified extreme values with a 95% confience interval. B) The two extreme values were excluded to remove their influence on the model, and 98% confience intervals were calculated. A third extreme value was identified and excluded from the analysis.
...
```{r Correction_to_Signal_98CI_graph, echo=FALSE, warning=F, message=F, fig.width = 6, out.width="70%", fig.asp=0.618, fig.align='center', fig.cap = "The relationship between side-by-side chlorophyll difference (corrections) and experiment chlorophyll difference (signal). Solid black line is a 1-1 model, dashed red lines are 98\\% confience intervals of the signal-to-correction relationship. Initial extreme values identified with 95\\% confience interval were removed to exclude their influence on the model. Intervals were recalculated with 98\\% confience to identify addition extreme values. A third extreme value was identified and excluded from the analysis.\\label{Corr_to_signal_98CI_graph}"}
### Remove two problem data points - remove their influence on confidence intervals
Corr_chl_diff_data1 = Corr_chl_diff_data[-c(21,23),]
...
Three filtration trials were conducted on flood tides, one on an ebb tide, and one control trial on an ebb tide (Table 1).
...
Deanza had the most field days with ten filtration trials and three controls all conducted on ebb tides. 
Three experimental trials were removed from the analysis because of their extreme values Figure \ref{Corr_to_signal_95CI_graph} and \ref{Corr_to_signal_98CI_graph}.
...
Shellmaker averaged `r round(mean(NPSM_filter_sum$pcnt_Chl_rmvd),1)` \% Chl $\alpha$ removed (*N* = `r nrow(NPSM_filter_sum)`, *SD* = `r round(sd(NPSM_filter_sum$pcnt_Chl_rmvd),1)`), and its control trial was removed from analysis as an extreme value.
...
data_analysis <- Final_table[-c(13, 15, 19),] # remove excluded analysis data points
...
sumtable_latex <- xtable(Latex_table_data, 
                         caption = "Whole-habitat in situ filtration and control trials across four California sites. Water quality values are upstream instrument averages. Depth, water velocity, Total Particulate Matter (TPM), and Organic Content (OC) are averages from multiple measurements or samples. The three observations separated at the bottom are extreme values not included in the analysis.",
                         label = "tb:summary",
                         align = "lllllrrccccccccr",
                         type = "latex")
...
Var_import1
Var_import1 <- Var_import2[order(Var_import2[,1], decreasing=TRUE),] # sort matrix highest to lowest - output names numeric
...
The Random Forest model that included all filtration and control trials (*R*^2^: `r round(r.sq,2)`) indicated that temperature had the highest relative importance to the model (`r round(Temp_rel_import, 2)`\%), followed by OC (`r round(OC_rel_import, 1)`\%), turbidity (`r round(Turb_rel_import, 1)`\%), TPM (`r round(TPM_rel_import, 1)`\%), salinity (`r round(Sal_rel_import, 1)`\%), and site (`r round(Site_rel_import, 1)`\%) respectively.
Filtration and control trials adjusted by the Random Forest model were not significantly different (two-sample *t*(`r t_test_adj_df`) = `r round(t_test_adj$statistic, digits = 2)`, *p* = `r round(t_test_adj$p.value, digits = 3)`); however, we expect our limited sample size inhibits statistical signal detection.

### Variables Affecting HCR

The random forest model with only filtration trials (*R*^2^: `r round(r.sq.filter, 2)`) indicated that temperature (`r round(Temp_rel_import_HCR,1)`\%) had the highest relative importance to the model, followed by turbidity (`r round(Turb_rel_import_HCR,1)`\%), TPM (`r round(TPM_rel_import_HCR,1)`\%), OC (`r round(OC_rel_import_HCR,1)`\%), site (`r round(Site_rel_import_HCR,1)`\%), and salinity (`r round(Sal_rel_import_HCR,1)`\%).
...
At Deanza we found a significant relationship between *O. lurida* shell length and DTW (ln(y) = `r round(coef(OlyDTW_loglog_sum)[1,1],digits=2)` `r round(coef(OlyDTW_loglog_sum)[2,1],digits=3)`ln(x), R^2^ = `r round(OlyDTW_loglog_sum$r.squared,digits=2)`, *p* = `r round(coef(OlyDTW_loglog_sum)[2,4],digits=3)`).
***I don't think this graph equation is populating correctly. Doesn't match model summary output***
*O. lurida* from different bays showed different allometric relationships? 
Dr. Nichols is writing code to see if the two data sets can use same model, or if we need two site specific models. 
...
```{r Oly_DTW_SH_outrmd_Graphs, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align="center", warning=F, message=F, fig.asp=0.618, fig.width=6, out.width="70%", fig.cap="The relationship between \\textit{O. lurida} shell length and its dry tissue weight (DTW) in Newport Bay, California at the Deanza restoration site. \\label{Oly_DTW_SH_out_rmd_Trans}"}
