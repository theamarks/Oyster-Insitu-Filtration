...
A statistical power analysis determind that the effect size between filtration and control trial HCR was large (*ES* = `r round(test1_power$d, 2)`), per Cohen's (1988) criteria, and there was little power (`r round(test1_power$power,2)`) to detect a statistical signal.
...
Individual water quality variables did not have a significant correlation with HCR at two quantiles: temperature ($\tau$ = 0.5, $\beta$±SE = `r round(sumQRtemp[[1]]$coefficients[2,2],2)`, *p* = `r round(sumQRtemp[[1]]$coefficients[2,4],2)`; $\tau$ = 0.9, $\beta$±SE = `r round(sumQRtemp[[2]]$coefficients[2,2],2)`, *p* = `r round(sumQRtemp[[2]]$coefficients[2,4],2)`), salinity ($\tau$ = 0.5, $\beta$±SE = `r round(sumQRsal[[1]]$coefficients[2,2],2)`, *p* = `r round(sumQRsal[[1]]$coefficients[2,4],2)`; $\tau$ = 0.9, $\beta$±SE = `r round(sumQRsal[[2]]$coefficients[2,2],2)`, *p* = `r round(sumQRsal[[2]]$coefficients[2,4],2)`),
