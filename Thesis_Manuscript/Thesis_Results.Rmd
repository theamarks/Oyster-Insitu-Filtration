...
print(sumtable_side_LaTex, include.rownames = FALSE)
#type = "latex",
...
test2$adjusted.clr = control.model$residuals

up.res = test2$adjusted.clr[test2$stream=="upstream"]
dn.res = test2$adjusted.clr[test2$stream=="downstream"]
m2 = lm(up.res~dn.res)
summary(m2)
dev.off()
plot(up.res,dn.res)
abline(0,1,col=4,lwd=2,lty=2)
x.seq = seq(min(dn.res),max(dn.res),by = (max(dn.res)-min(dn.res))/1000)
conf.seq = predict.lm(m2,newdata=data.frame(dn.res=x.seq),interval="confidence")
...
```{r Bivalve_Density, echo=FALSE, fig.align="center", fig.asp=0.618, fig.width=6, warning= F, message= F, fig.keep='all', dev='pdf', out.width="70%", fig.cap="The density of bivlaves at Deanza Newport, California surveyed May 2018. \\label{Bivalve_Density}"}
...
Oly_TWW_DTW_rm_out <- ggplot(NPD_oly_TWW, aes(total_wet_weight_g, tissue_dry_weight_g)) +
  ...
  geom_point(aes(color = species)) +
  scale_color_manual(values = Species_colors, guide=FALSE) +
  stat_function(fun = Gray17_DTW_TWW_lm(NPD_oly_TWW$total_wet_weight_g), colour = "blue", linetype="dashed") +
  xlim(0, 7) +
  ylim(0,0.3) +
  theme_classic() +
  labs(x = 'Total wet weight (g)',
