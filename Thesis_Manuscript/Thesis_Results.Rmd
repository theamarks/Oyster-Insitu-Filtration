...
```{r Site WQ ANOVA, fig.width=7, fig.asp=1.2, fig.cap="Box plots of ambient (upstream) A) temperature, B) salinity, C) turbidity, D) total particulate matter, E) organic content, and F) chlorophyll $\\alpha$ from filtration and control trials. Lower case letters indicate post-hoc Tukey results among sites. Trials were conducted from February 2018 to June 2019 at San Rafael, CA (restored reefs); Morro Bay, CA (Morro Bay Oyster Company, aquaculture); and Newport Bay, CA (Shellmaker and Deanza, restored beds). \\label{Site_WQ_boxplot}" }
...
Temp_box <- ggplot(site_data_outrmd, aes(Site, Temp_C_Up)) +
  ...
  annotate("text", "San Rafael", 
           max((site_data_outrmd %>% 
                  filter(Site == "San Rafael"))$Temp_C_Up) + 0.5, label = "a") +
  annotate("text", "Morro Bay", 
           max((site_data_outrmd %>% 
                  filter(Site == "Morro Bay"))$Temp_C_Up) + 0.5, label = "a") +
  annotate("text", "Shellmaker", 
           max((site_data_outrmd %>% 
                  filter(Site == "Shellmaker"))$Temp_C_Up) + 0.5, label = "a") +
  annotate("text", "Deanza", 
           max((site_data_outrmd %>% 
                  filter(Site == "Deanza"))$Temp_C_Up) + 0.5, label = "a") 
...
Sal_box <- ggplot(site_data_outrmd, aes(Site, Sal_ppt_Up)) +
  ...
  annotate("text", "San Rafael", 
           max((site_data_outrmd %>% 
                  filter(Site == "San Rafael"))$Sal_ppt_Up) + 0.5, label = "a") +
  annotate("text", "Morro Bay", 
           max((site_data_outrmd %>% 
                  filter(Site == "Morro Bay"))$Sal_ppt_Up) + 0.5, label = "b") +
  annotate("text", "Shellmaker", 
           max((site_data_outrmd %>% 
                  filter(Site == "Shellmaker"))$Sal_ppt_Up) + 0.5, label = "b") +
  annotate("text", "Deanza", 
           max((site_data_outrmd %>% 
                  filter(Site == "Deanza"))$Sal_ppt_Up) + 0.5, label = "b") 
...
OC_box <- ggplot(site_data_outrmd, aes(Site, Avg_OC_Ratio)) +
  ...
  annotate("text", "Morro Bay", 
           max((site_data_outrmd %>% 
                  na.omit() %>% 
                  filter(Site == "Morro Bay"))$Avg_OC_Ratio) + 0.025, label = "b") +
  annotate("text", "Shellmaker", 
           max((site_data_outrmd %>% 
  ...
  annotate("text", "Deanza", 
           max((site_data_outrmd %>% 
                  na.omit() %>% 
                  filter(Site == "Deanza"))$Avg_OC_Ratio) + 0.025, label = "b") 
...
Chl_box <- ggplot(site_data_outrmd, aes(Site, Chl_ug_L_Up)) +
  ...
  annotate("text", "San Rafael", 
           max((site_data_outrmd %>% 
                  filter(Site == "San Rafael"))$Chl_ug_L_Up) + 0.5, label = "a") +
  annotate("text", "Morro Bay", 
           max((site_data_outrmd %>% 
                  filter(Site == "Morro Bay"))$Chl_ug_L_Up) + 0.5, label = "ab") +
  annotate("text", "Shellmaker", 
           max((site_data_outrmd %>% 
                  filter(Site == "Shellmaker"))$Chl_ug_L_Up) + 0.5, label = "b") +
  annotate("text", "Deanza", 
           max((site_data_outrmd %>% 
                  filter(Site == "Deanza"))$Chl_ug_L_Up) + 0.5, label = "ab")
...
########### Power analysis - sensitivity power analysis 
# one way to estimate Cohen’s d
# d = effect size = abs(mean(sample1) - mean(sample2)) / root mean square standard deviation
d_adj_test1 <- {abs(mean(y.adj.experiment) - mean(y.adj.control)) / 
  sqrt((sd(y.adj.experiment)^2 + sd(y.adj.control)^2) / 2)}
...
A statistical power analysis determind that the effect size between filtration and control trial HCR was large (*ES* = `r round(test1_power$d, 2)`), per Cohen's (1988) criteria, and there was little power (`r round(test1_power$power,2)`) to detect a statistical signal with my relatively small sample size.
A random forest model including all filtration and control trials (*R*^2^ = `r round(r.sq,2)`) indicated that temperature had the highest relative importance to the model (`r round(Temp_rel_import, 1)`\%), followed by OC (`r round(OC_rel_import, 1)`\%), turbidity (`r format(round(Turb_rel_import, 1), nsmall = 1)`\%), TPM (`r format(round(TPM_rel_import, 1), nsmall =1)`\%), salinity (`r format(round(Sal_rel_import, 1), nsmall = 1)`\%), and site (`r format(round(Site_rel_import, 1), nsmall =1)`\%).
The residuals of this random forest model were used to adjust HCR values to account for water quality variables and site. 
...
A second statistical power analysis determined that the effect size of adjusted HCRs between filtration and control trials was greater (*ES* = `r round(adj_test1_power$d,2 )`) than the unadjusted t-test along with more power (`r round(adj_test1_power$power, 2)`).
Despite non-significant comparisons between control and filtration trials, statistical power anlaysis indicate that my limited sample size is inhibiting statistical inference of a large effect size of oyster habitat HCRs compared to mudflat controls.

## Habitat Clearance Rates 

HCR at Ran Rafael averaged `r round(mean(SR_filter_sum$L_hr_m2),1)` L hr^-1^ m^-2^ (*N* = `r nrow(SR_filter_sum)`, *SD* = `r round(sd(SR_filter_sum$L_hr_m2),1)`), 
...
and at Deanza averaged `r round(mean(NPD_filter_sum$L_hr_m2),1)` L hr^-1^ m^-2^ (*N* = `r nrow(NPD_filter_sum)`, *SD* = `r round(sd(NPD_filter_sum$L_hr_m2),1)`) (Figure \ref{FR_Site_boxplot}).
Control trials measured `r round(SR_NC_sum$L_hr_m2,1)`, `r round(MB_NC_sum$L_hr_m2,1)`, `r round(NPSM_NC_sum$L_hr_m2,1)`, and `r round(NPD_NC_sum$L_hr_m2,1)` L hr^-1^ m^2^ for San Rafael, and Morro Bay, Shellmaker, and Deanza control trials respectfully.
HCR for filtration trials did not differ significantly among sites (one-way ANOVA, (*F*(3,16) = 0.29, *p* = 0.83).
Individual water quality variables did not have a significant correlation with HCR at 0.5 and 0.9 quantiles: temperature ($\tau$ = 0.5, $\beta$±*SE* = `r round(sumQRtemp[[1]]$coefficients[2,2],2)`, *p* = `r round(sumQRtemp[[1]]$coefficients[2,4],2)`; $\tau$ = 0.9, $\beta$±*SE* = `r round(sumQRtemp[[2]]$coefficients[2,2],2)`, *p* = `r round(sumQRtemp[[2]]$coefficients[2,4],2)`), salinity ($\tau$ = 0.5, $\beta$±*SE* = `r round(sumQRsal[[1]]$coefficients[2,2],2)`, *p* = `r round(sumQRsal[[1]]$coefficients[2,4],2)`; $\tau$ = 0.9, $\beta$±*SE* = `r round(sumQRsal[[2]]$coefficients[2,2],2)`, *p* = `r round(sumQRsal[[2]]$coefficients[2,4],2)`),
