...
```{r eval=FALSE, fig.height=6, fig.width=6,  include=FALSE}
# This model makes a lot of sense. DTW will never be negative whill length is positive 
Oly_LogDTW_lm <- lm(data = NPD_oly_outRmd, log(tissue_dry_weight_g) ~ length_mm)
...
# copied code from Dr. Nichols to extract model attributes. Do I need to exp() all of these?
DTW_slope = exp(summary(Oly_LogDTW_logSH_lm)[[4]][1])
...
Oly_loglogDTWSH_model <- function(x){DTW_slope*x + exp(coef(Oly_LogDTW_logSH_lm)[[1]])}
...
SR_Oly_DTW_data <- SR_Oly_Shelllength_data %>% 
  mutate(DTW_g = Oly_DTW_SL_model(length_mm)) %>%  # predict Oly Dry tissue weight with length_mm
...
SR_NPD_biv_DTWm2 <- rbind(SR_Oly_DTW_data, NPD_biv_DTW_m2)
