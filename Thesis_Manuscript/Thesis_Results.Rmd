...
```{r FRpos_TPM_LM, cache=TRUE, include=FALSE}
FRPos_TPM_lm <- lm(data = Filtration_pos_data, L_hr_m2 ~ Avg_TPM)
#summary(FR_TPM_lm)
ols_regress(FRPos_TPM_lm)
```

```{r ChlRmdpos_TPM_LM, cache=TRUE, include=FALSE}
...
```{r fig.width=12, , fig.height=10, cache=TRUE, include=FALSE}
Chl_RmdPos_TPM_graph <- ggplot((Filtration_pos_data), aes(Avg_TPM, pcnt_Chl_rmvd)) +
  geom_smooth(method = lm, se = T, level = 0.95, color = 'grey50') +
  geom_point(aes(color = Site)) +
  # geom_abline(aes(intercept=0, slope=1)) +
...
```{r NPD_FRPos_TPM_graph, fig.height=5, fig.width=12, cache=TRUE, include=FALSE}
...
```{r fig.height=6, fig.width=6, cache=T, include=FALSE}
...
```{r FRPos vs PIM, fig.height=5, fig.width=14, cache=TRUE, include=FALSE}
...
```{r Deanza_Filter_Pos_PIM, fig.height=5, fig.width=12, cache=TRUE, include=FALSE}
...
```{r FRPos vs OC, fig.height=5, fig.width=15, cache=TRUE, include=FALSE}
...
### ***Y ~ X Diagnostics***
```{r fig.height=10, fig.width=15, cache=TRUE, include=FALSE}
par(mfrow=c(2,2))
...
```{r LogChlRmdPos_OC, fig.height=5, fig.width=6, cache=TRUE, include=FALSE}
...
```{r Deanza_Filter_Pos_OC, fig.height=5, fig.width=12, cache=TRUE, include=FALSE}
...
### ***TPM by OC***
```{r TPM_OC, fig.height=10, fig.width=14, cache=TRUE, include=FALSE}
