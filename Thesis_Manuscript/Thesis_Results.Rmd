...
### Test extreme value removal
extreme_test <- Filter_only_outRmd_data %>% 
  filter(!abs(L_hr_m2) > 2000)

ext_test_sum <- extreme_test %>%
  group_by(Site) %>% 
  summarize(avg_HCR = mean(L_hr_m2),
            SD = sd(L_hr_m2),
            n_samples = length(L_hr_m2),
...
HCR_site_aov_noext <- aov(L_hr_m2 ~ Site, data = extreme_test)
summary(HCR_site_aov_noext)
