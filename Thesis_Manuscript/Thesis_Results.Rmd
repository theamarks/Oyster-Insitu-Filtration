...
Percent Chl $\alpha$ removal at the San Rafael *O. lurida* reef ranged from `r round(min(SR_filter_sum$pcnt_Chl_rmvd),1)`\% to `r round(max(SR_filter_sum$pcnt_Chl_rmvd),1)`\% and recorded `r round(SR_NC_sum$pcnt_Chl_rmvd,1)`\% in the negative control over bare mudflat. Morro Bay *C. gigas* aquaculture lines ranged from `r round(min(MB_filter_sum$pcnt_Chl_rmvd),1)`\% to `r round(max(MB_filter_sum$pcnt_Chl_rmvd),1)`\% and over bare mud flat `r round(MB_NC_sum$pcnt_Chl_rmvd,1)`\%. At the Newport Bay Deanza *O. lurida* bed, Chl $\alpha$ removal ranged from `r round(min(NPD_filter_sum$pcnt_Chl_rmvd),1)`\% to `r round(max(SR_filter_sum$pcnt_Chl_rmvd),1)`\% and recorded `r round(SR_NC_sum$pcnt_Chl_rmvd,1)`\% in the negative control
