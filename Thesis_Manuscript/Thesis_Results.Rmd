...
Total Particulate Matter (TPM) is a significant predictor of seston OC when sites are combined (ln(y) = `r round(coef(logOC_logTPM_LM_sum)[1,1],digits=2)` `r round(coef(logOC_logTPM_LM_sum)[2,1],digits=3)` ln(x), R^2^ = 0.30. *p* = `r round(coef(logOC_logTPM_LM_sum)[2,4],digits = 3)` (Figure \ref{TPM_OC_model}). 
...
At Deanza we found *O. lurida* shell length best predicted DTW in a natural log-linear model (ln(y) = -5.76 + 0.096x, R*^2^*= 0.74).
...
```{r Compare_NPD_DTW_SL_transformations, eval=FALSE, include=FALSE}
...
```{r Oly_DTW_SH_out_rmd_Trans, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align="center", warning=F, message=F, fig.asp=0.618, fig.width=6, out.width="70%", fig.cap="The relationship between \\textit{O. lurida} shell length and its dry tissue weight (DTW) in Newport Bay, California at the Deanza restoration site. \\label{Oly_DTW_SH_out_rmd_Trans}"}
