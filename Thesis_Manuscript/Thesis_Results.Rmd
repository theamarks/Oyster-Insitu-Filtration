...
```{r Site Summary Table}
Site_Sum <- Final_table %>% 
  filter(!Date == "2019-04-17") %>% 
  dplyr::group_by(Site) %>% 
  dplyr::summarize(Temp = paste0(round(mean(na.omit(Temp_C_Up)),2),
                                 ' (SD = ', 
...
site_sum_table %<>% 
  dplyr::select(Site, Lat, Long, Temp, Sal, Turb, TPM, OC, Months) %>% 
