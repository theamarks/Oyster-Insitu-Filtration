...
```{r Site WQ ANOVA, fig.width=7, fig.asp=1.3, fig.cap="Box plots of ambient (upstream) water quality variables from filtration and control trials. Trials were conducted from February 2018 to June 2019 at San Rafael, CA (restored reefs); Morro Bay, CA (Morro Bay Oyster Companyaquaculture); and Newport Bay, CA (Shellmaker and Deanza, restored beds). \\label{Site_WQ_boxplot}" }
#########################
###### Chlorphyll
#hist(site_data_outrmd$Chl_ug_L_Up)
site_chl_aov <- aov(Chl_ug_L_Up ~ Site, data = site_data_outrmd)
#summary(site_chl_aov)
#broom::tidy(site_chl_aov) # put ANOVA model into tibble to extract individual values
# Post-Hoc Tukey test
...
Chl_box <- ggplot(site_data_outrmd, aes(Site, Chl_ug_L_Up)) +
  ...
  theme_classic() +
  labs(x = 'Site',
       y = expression(paste("Upstream Chlorophyll ",alpha))) +
...
#broom::tidy(site_temp_aov) # put ANOVA model into tibble to extract individual values
# Post-Hoc Tukey test
