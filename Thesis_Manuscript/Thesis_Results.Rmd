...
```{r ChlRmd_Site_boxplot, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align= "center", fig.asp=0.618, out.width="70%", message= F, fig.cap="Box plots of percent chlorophyll $\\alpha$ removal during filtration trials, control trials are not displayed. Each data point is an the average value from a single filtration trial. Filtration trials were conducted between February 2018 to June 2019 at San Rafael, CA (restored reefs); Morro Bay, CA (Morro Bay Oyster Company aquaculture); and Newport Bay, CA (Shellmaker and Deanza, restored beds). Each data point is the average value from a single filtration trial. Extreme values excluded from analysis are not shown. \\label{ChlRmd_Site_boxplot}"}
...
HCR_lm <- lm(qr_y ~ qr_x)
summary(HCR_lm)
hist(Filtration_only_data$L_hr_m2)
...
# Temperature plot
qrplot_temp <- ggplot(Filtration_only_data, aes(Temp_C_Up, L_hr_m2)) +
  geom_point() +
  geom_quantile(quantiles = q4)
qrplot_temp 
# Salinity plot
qrplot_sal <- ggplot(Filtration_only_data, aes(Sal_ppt_Up, L_hr_m2)) +
  geom_point() +
  geom_quantile(quantiles = q4)
qrplot_sal
# Turbidity plot
qrplot_turb <- ggplot(Filtration_only_data, aes(Turbidity_NTU_Up, L_hr_m2)) +
  geom_point() +
  geom_quantile(quantiles = q4)
qrplot_turb
# TPM plot
qrplot_tpm <- ggplot(Filtration_only_data, aes(Avg_TPM_mg_L, L_hr_m2)) +
  geom_point() +
  geom_quantile(quantiles = q4)
qrplot_tpm
# OC plot
qrplot_oc <- ggplot(Filtration_only_data, aes(Avg_OC_Ratio, L_hr_m2)) +
  geom_point() +
  geom_quantile(quantiles = q4)
qrplot_oc
