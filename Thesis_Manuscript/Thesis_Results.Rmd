...
SR_NC_sum <- Filter_only_outRmd_data %>% 
  filter(Site %in% c("San Rafael"),
...
MB_NC_sum <- Filter_only_outRmd_data %>% 
  filter(Site %in% c("Morro Bay"),
         grepl("Neg Control*", Experiment)) 
...
NPD_NC_sum <- Filter_only_outRmd_data %>% 
  filter(Site %in% c("Deanza"),
         grepl("Neg Control*", Experiment)) 
...
NPSM_NC_sum <- Filter_only_outRmd_data %>% 
  filter(Site %in% c("Shellmaker"),
         grepl("Neg Control*", Experiment)) 
...
Field work was conducted from February 2018 to June 2019 and measured 22 filtration trials and 6 control trials (Table 1).
Filtration trials at San Rafael were conducted on the ebb and flood tides surrounding low tide.
Three filtration trials were conducted on flood tides (2018/7/7, 2018/7/18, 2018/9/10), one on an ebb tide (2018/11/8), and one control trial on an ebb tide (2018/11/7).
We recorded four filtration trials and one control at Morro Bay, all conducted on flood tides.
At Shellmaker we recorded four filtration trials and one control on flood tides.
Deanza had the most field days with 10 filtration trials and 3 controls all conducted on ebb tides. 

### Percent chlorophyll $\alpha$ removal

Percent Chl $\alpha$ removed at San Rafael averaged `r round(mean(SR_filter_sum$pcnt_Chl_rmvd),1)`\% (n = `r nrow(SR_filter_sum)`, SD = `r round(sd(SR_filter_sum$pcnt_Chl_rmvd),2)`; Figure \ref{ChlRmd_Site_boxplot}) and recorded `r round(SR_NC_sum$pcnt_Chl_rmvd,1)`\% in the single control trial. 
Morro Bay averaged `r round(mean(MB_filter_sum$pcnt_Chl_rmvd),1)`\% Chl $\alpha$ removed (n = `r nrow(MB_filter_sum)`, SD = `r round(sd(MB_filter_sum$pcnt_Chl_rmvd),1)`) and measured `r round(MB_NC_sum$pcnt_Chl_rmvd,1)`\% during the control trial. 
At Deanza Chl $\alpha$ removed averaged `r round(mean(NPD_filter_sum$pcnt_Chl_rmvd),1)`\% (n = `r nrow(NPD_filter_sum)`, SD =  `r round(sd(NPD_filter_sum$pcnt_Chl_rmvd),1)`) and during three consecutive control trials averaged `r round(mean(NPD_NC_sum$pcnt_Chl_rmvd))`\% Chl $\alpha$ removed (n = `r nrow(NPD_NC_sum)`, SD =  `r round(sd(NPD_NC_sum$pcnt_Chl_rmvd))`).
Shellmaker averaged `r round(mean(NPSM_filter_sum$pcnt_Chl_rmvd),1)`\% Chl $\alpha$ removed (n = `r nrow(NPSM_filter_sum)`, SD = `r round(sd(NPSM_filter_sum$pcnt_Chl_rmvd),1)`) and `r round(NPSM_NC_sum$pcnt_Chl_rmvd,1)`\% Chl $\alpha$ removal in the single control. 

```{r ChlRmd_Site_boxplot, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align= "center", fig.show="hold", fig.asp=0.618, out.width="70%", message= F, fig.cap="Percent chlorophyll removed during filtration trials. Each data point is an the average value from the experiment. Filtration trials were conducted between February 2018 to June 2019 at San Rafael, CA (restored \\textit{O. lurida} reefs); Morro Bay, CA (Morro Bay Oyster Company \\textit{C. gigas} aquaculture); and Newport Bay, CA that contained Shellmaker and Deanza sites (both restored \\textit{O. lurida} shell beds) Extreme values excluded from analysis are not shown. \\label{ChlRmd_Site_boxplot}"}
...
HCR at Ran Rafael average `r round(mean(SR_filter_sum$L_hr_m2),1)` Lhr^-1^m^2^ (n = `r nrow(SR_filter_sum)`, SD = `r round(sd(SR_filter_sum$L_hr_m2),1)`; Figure \ref{FR_Site_boxplot}). 
Morro Bay averaged `r round(mean(MB_filter_sum$L_hr_m2),1)` Lhr^-1^m^2^ (n = to `r nrow(MB_filter_sum)`, SD = `r round(sd(MB_filter_sum$L_hr_m2),1)`).
Shellmaker HCR averaged `r round(mean(NPSM_filter_sum$L_hr_m2),1)` Lhr^-1^m^2^ (n = `r nrow(NPSM_filter_sum)`, SD = `r round(sd(NPSM_filter_sum$L_hr_m2),1)`).
Deanza's HCR average was `r round(mean(NPD_filter_sum$L_hr_m2),1)` Lhr^-1^m^2^ (n = `r nrow(NPD_filter_sum)`, SD = `r round(sd(NPD_filter_sum$L_hr_m2),1)`).
Control trials measured `r round(SR_NC_sum$L_hr_m2,1)`, `r round(MB_NC_sum$L_hr_m2,1)`, and `r round(NPSM_NC_sum$L_hr_m2,1)` Lhr^-1^m^2^ for San Rafael, Morro Bay, and Shellmaker respectively.
Newport Deanza's three consecutive control trials averaged `r round(mean(NPD_NC_sum$L_hr_m2))` Lhr^-1^m^2^ (n = `r nrow(NPD_NC_sum$L_hr_m2)`, SD = `r round(sd(NPD_NC_sum$L_hr_m2),1)`).

```{r FR_Site_boxplot, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.show="hold", fig.align= "center", fig.asp=0.618, out.width="70%", message= F, fig.cap="The habitat clearance rate during filtration trials. Each data point is an the average value from the experiment. Filtration trials were conducted between February 2018 to June 2019 at San Rafael, CA (restored \\textit{O. lurida} reefs); Morro Bay, CA (Morro Bay Oyster Company \\textit{C. gigas} aquaculture); and Newport Bay, CA that contained Shellmaker and Deanza sites (both restored \\textit{O. lurida} shell beds) Extreme values excluded from analysis are not shown. \\label{FR_Site_boxplot}"}
...
### Summary Table Here - Still broken

\begin{sidewaystable}[ht]
\centering
\begin{tabular}{rlllrrrrrrrrrrrr}
  \toprule
 & Site & Date & Experiment & Temp (\textdegree C) & Salinity (ppt) & Turbidity (NTU) & TPM (mg/L) & OC & Chl \alpha up (\mu g/L) & Chl \alpha down (\mu g/L) & Depth (m) & Distance (m) & Water velocity (m/s) & % Chl \alpha removal & Clearance rate (L/hr/m^2^) \\ 
  \midrule
1 & Deanza & 2018-10-25 & Filtration & 21.09 & 33.50 & 4.80 & NA & NA & 3.20 & 3.40 & 0.18 & 19.20 & 0.06 & -10.10 & -216.00 \\ 
  2 & Deanza & 2018-10-26 & Filtration & 20.96 & 36.40 & 2.40 & 4.39 & 0.36 & 4.00 & 3.30 & 0.19 & 19.20 & 0.08 & 16.40 & 467.00 \\ 
  3 & Deanza & 2019-04-15 & Filtration & 19.43 & 36.40 & 3.40 & 5.07 & 0.36 & 1.30 & 1.30 & 0.23 & 16.40 & 0.07 & -0.70 & -22.00 \\ 
  4 & Deanza & 2019-04-17 & Filtration & 19.41 & 37.40 & 4.90 & 4.03 & 0.23 & 0.10 & 1.10 & 0.18 & 16.40 & 0.11 & -108.90 & -4867.00 \\ 
  5 & Deanza & 2019-05-09 & Filtration & 19.44 & 37.00 & 2.30 & 2.67 & 0.21 & 5.40 & 5.20 & 0.32 & 15.00 & 0.07 & 4.60 & 242.00 \\ 
  6 & Deanza & 2019-05-10 & Filtration & 19.25 & 37.40 & 2.90 & 5.10 & 0.16 & 4.00 & 3.80 & 0.31 & 14.80 & 0.09 & 5.60 & 383.00 \\ 
  7 & Deanza & 2019-05-11 & Filtration & 18.85 & 36.50 & 2.90 & 4.00 & 0.24 & 3.70 & 3.60 & 0.35 & 15.00 & 0.10 & 2.90 & 248.00 \\ 
  8 & Deanza & 2019-05-21 & Filtration & 18.89 & 34.20 & 3.30 & 3.57 & 0.23 & 6.30 & 6.50 & 0.24 & 16.00 & 0.08 & -3.50 & -153.00 \\ 
  9 & Deanza & 2019-05-22 & Filtration & 18.62 & 33.90 & 2.80 & 3.83 & 0.23 & 7.40 & 7.00 & 0.22 & 15.70 & 0.06 & 4.80 & 149.00 \\ 
  10 & Deanza & 2019-06-08 & Filtration & 19.07 & 37.10 & 2.70 & 3.65 & 0.27 & 4.30 & 4.40 & 0.26 & 15.70 & 0.09 & -2.80 & -156.00 \\ 
  11 & Deanza & 2019-06-09 & Neg Control 1 & 19.18 & 37.10 & 1.30 & 2.80 & 0.21 & 2.80 & 2.80 & 0.48 & 12.80 & 0.12 & -2.90 & -456.00 \\ 
  12 & Deanza & 2019-06-09 & Neg Control 2 & 19.22 & 37.00 & 2.00 & 2.80 & 0.21 & 2.90 & 2.90 & 0.33 & 12.80 & 0.06 & -3.40 & -199.00 \\ 
  13 & Deanza & 2019-06-09 & Neg Control 3 & 19.62 & 36.60 & 2.20 & 2.80 & 0.21 & 3.70 & 3.50 & 0.21 & 12.20 & 0.06 & 2.90 & 110.00 \\ 
  14 & Morro Bay & 2018-07-27 & Filtration & 17.66 & 35.90 & 7.90 & 17.53 & 0.26 & 4.50 & 3.70 & 0.48 & 26.00 & 0.03 & 17.50 & 346.00 \\ 
  15 & Morro Bay & 2018-07-28 & Filtration & 17.45 & 35.90 & 7.40 & 14.60 & 0.33 & 3.00 & 2.80 & 0.30 & 27.00 & 0.03 & 6.20 & 65.00 \\ 
  16 & Morro Bay & 2019-05-18 & Filtration & 16.29 & 37.60 & 17.50 & NA & NA & 3.00 & 3.00 & 0.42 & 30.50 & 0.08 & -3.40 & -128.00 \\ 
  17 & Morro Bay & 2019-05-19 & Filtration & 15.94 & 36.40 & 10.90 & 14.45 & 0.18 & 2.20 & 2.50 & 0.45 & 30.50 & 0.02 & -18.20 & -242.00 \\ 
  18 & Morro Bay & 2019-06-06 & Neg Control 1 & 17.28 & 38.10 & 11.90 & 12.80 & 0.22 & 2.10 & 2.10 & 0.32 & 20.50 & 0.05 & -0.70 & -19.00 \\ 
  19 & San Diego & 2018-10-17 & Filtration & 21.30 & 36.30 & 0.00 & NA & NA & 5.90 & 5.80 & 0.01 & 4.00 & 0.04 & 1.60 & 7.00 \\ 
  20 & San Rafael & 2018-07-17 & Filtration & 24.51 & 29.60 & 30.60 & 76.91 & 0.12 & 4.40 & 4.50 & 0.72 & 21.00 & 0.03 & -4.50 & -166.00 \\ 
  21 & San Rafael & 2018-07-18 & Filtration & 24.32 & 29.90 & 21.70 & 38.59 & 0.20 & 4.50 & 4.40 & 0.79 & 21.00 & 0.20 & 1.10 & 302.00 \\ 
  22 & San Rafael & 2018-09-10 & Filtration & 19.02 & 30.30 & 23.10 & 36.19 & 0.10 & 4.90 & 4.80 & 0.89 & 25.30 & 0.05 & 1.90 & 112.00 \\ 
  23 & San Rafael & 2018-11-07 & Neg Control 1 & 16.45 & 29.90 & 25.50 & 30.04 & 0.17 & 6.90 & 7.00 & 0.40 & 11.60 & 0.06 & -1.30 & -99.00 \\ 
  24 & San Rafael & 2018-11-08 & Filtration & 14.52 & 29.90 & 30.90 & 32.45 & 0.14 & 5.10 & 4.80 & 0.69 & 12.30 & 0.03 & 6.20 & 417.00 \\ 
  25 & Shellmaker & 2019-05-10 & Filtration & 19.78 & 37.00 & 4.10 & 5.90 & 0.24 & 3.70 & 3.80 & 0.38 & 13.00 & 0.06 & -1.90 & -120.00 \\ 
  26 & Shellmaker & 2019-05-11 & Filtration & 21.52 & 36.50 & 4.70 & 5.14 & 0.25 & 2.40 & 1.90 & 0.32 & 12.20 & 0.04 & 19.60 & 845.00 \\ 
  27 & Shellmaker & 2019-05-22 & Filtration & 19.29 & 33.70 & 3.80 & 4.92 & 0.20 & 2.70 & 4.10 & 0.26 & 8.60 & 0.04 & -60.30 & -2485.00 \\ 
  28 & Shellmaker & 2019-06-08 & Filtration & 20.06 & 36.00 & 4.00 & 3.95 & 0.20 & 2.70 & 2.70 & 0.26 & 8.60 & 0.04 & -2.10 & -97.00 \\ 
  29 & Shellmaker & 2019-06-09 & Neg Control 1 & 22.40 & 36.00 & 5.90 & 4.02 & 0.18 & 1.10 & 3.00 & 0.30 & 11.20 & 0.04 & -203.80 & -8844.00 \\ 
   \bottomrule
\end{tabular}
\label{tb:summary}
\end{sidewaystable}
...
colnames(Final_table) <- c( "Site", "Date", "Experiment", "Temp (\\textdegreeC)",
                            "Salinity (ppt)", "Turbidity (NTU)","TPM (mg/L)", "OC", 
                            "Chl \\alpha up (\\mu g/L)", 
                            "Chl \\alpha$ down (\\mu g/L)", 
                           "Depth (m)","Distance (m)", "Water velocity (m/s)", 
...
```{r Chl_Corr_Filter_diff_graph, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.show="hold", fig.align= "center", fig.asp=0.618, out.width="70%", message= F, warning=F, fig.cap="The relationship between side-by-side corrections and the chlorophyll difference (signal). Identified extreme values were removed from analysis \\label{SbsCorr_ChlDiff_graph}"}
...
Filtration trials had significantly higher HCRs (habitat clearance rate) than negative controls (two-sample t(`r t_test_df`) = `r round(filter_control_ttest$statistic, digits = 2)`, p = `r round(filter_control_ttest$p.value, digits = 2)`).
The Random Forest model that included all filtration and control trials indicated that temperature had the highest relative importance to the model (`r round(Temp_rel_import, 2)`\%), followed by OC (`r round(OC_rel_import, 1)`\%), turbidity (`r round(Turb_rel_import, 1)`\%), TPM (`r round(TPM_rel_import, 1)`\%), salinity (`r round(Sal_rel_import, 1)`\%), and site (`r round(Site_rel_import, 1)`\%) respectively (DO I Need these relative importance numbers if the point is to determine the diff between experiment and control?).
Filtration and control trials adjusted by the Random Forest model were not significantly different (two-sample t(`r t_test_adj_df`) = `r round(t_test_adj$statistic, digits = 2)`, p = `r round(t_test_adj$p.value, digits = 3)`).

### Variables Affecting HCR

The random forest model built with only filtration trials indicated that temperature (`r round(Temp_rel_import_HCR,1)`\%) had the highest relative importance to the model, followed by turbidity (`r round(Turb_rel_import_HCR,1)`\%), TPM (`r round(TPM_rel_import_HCR,1)`\%), OC (`r round(OC_rel_import_HCR,1)`\%), site (`r round(Site_rel_import_HCR,1)`\%), and salinity (`r round(Sal_rel_import_HCR,1)`\%).
...
Total Particulate Matter (TPM) is a weak predictor seston OC when sites are combined (Need to pick model, report here) (Figure \ref{TPM_OC_models}). 
...
Bays have distinct TPM signatures that do not overlap (Figure \ref{TPM_OC_models}), while OC remains relatively constant across bay.
Northern San Francisco Bay (San Rafael site) TPM averaged `r round(mean(SR_TPM_OC$Avg_TPM_mg_L),2)` mg/L (n = `r nrow(SR_TPM_OC)`, SD = `r round(sd(SR_TPM_OC$Avg_TPM_mg_L),2)`, and Morro Bay TPM averaged `r round(mean(MB_TPM_OC$Avg_TPM_mg_L),2)` mg/L (n = `r nrow(MB_TPM_OC)`, SD = `r round(sd(MB_TPM_OC$Avg_TPM_mg_L),2)`).
Newport Bay (Deanza and Shellmaker sites) TPM averaged `r round(mean(NP_combo_TPM_OC$Avg_TPM_mg_L),3)` mg/L (n = `r nrow(NP_combo_TPM_OC)`, SD = `r round(sd(NP_combo_TPM_OC$Avg_TPM_mg_L),2)`).
...
In November 2017 the estimated bivalve density at San Rafael was `r SR_Den` individuals / m^2^, all bivalves were *O. lurida* (Figure \ref{Bivalve_Den_Biomass}).
Other bivalves were noted, but were not measured in sample bags (Zabin unpublished data?). 
Morro Bay has an estimated `r MB_Den` *C. gigas* individuals / m^2^ (Morro Bay Oyster Company), personal field observations confirm the lack of bivalve fouling on the aquaculture lines.
Shellmaker has an estimated `r NPSM_Den` individuals / m^2^, composed of *Adula diegensis* (`r NPSM_species[NPSM_species$Species=="Adula diegensis",2]` individuals / m^2^), *Musculista senhousia* (`r NPSM_species[NPSM_species$Species=="Musculista senhousia",2]` individuals / m^2^), *O. lurida* (`r NPSM_species[NPSM_species$Species=="Ostrea lurida",2]` individuals / m^2^), *Mytilus galloprovincialis* (`r NPSM_species[NPSM_species$Species=="Mytilus galloprovincialis",2]` individuals / m^2^), *Geukensia demissa* (`r NPSM_species[NPSM_species$Species=="Geukensia demissa",2]` individuals / m^2^), and *Argopecten ventricosa* (`r NPSM_species[NPSM_species$Species=="Argopecten ventricosa",2]` individuals / m^2^).
Deanza has an estimated `r NPD_Den` individuals / m^2^, composed of *M. senhousia* (`r NPD_species[NPD_species$Species=="Musculista senhousia",2]`), *A. diegensis* (`r NPD_species[NPD_species$Species=="Adula diegensis",2]`), *O. lurida* (`r NPD_species[NPD_species$Species=="Ostrea lurida",2]`), *M. galloprovincialis* (`r NPD_species[NPD_species$Species=="Mytilus galloprovincialis",2]`).
...
We found *O. lurida* shell length best predicted DTW in a log-log model (equation here, and need to determine best fit).
*O. lurida* from different bays showed different allometric relationships? 
How to compare two non-linear models that have different ranges? 

```{r Oly_DTW_SH_outlier_rmd, echo=FALSE, fig.keep = 'all', dev = 'pdf', warning=F, message=F, fig.align="center", out.width = "70%", fig.asp=0.618, fig.width=6, fig.cap="The relationship between \\textit{O. lurida} shell length and its dry tissue weight (DTW) at two locations, Newport Bay, California (Zacherl unpublished data) and Yaquina Bay, Oregon (Gray unpublished data). The Newport Bay data was collected at the Deanza restoration site and outlier data (outside mean +- 2.2 SD) and negative values are excluded. \\label{Oly_DTW_SH_outlier_rmd}"}
...
```{r Oly_DTW_SH_out_rmd_Trans, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align="default", warning=F, message=F, fig.asp=0.618, fig.width=8, fig.cap="The relationship between \\textit{O. lurida} shell length and its dry tissue weight (DTW) in Newport Bay, California at the Deanza restoration site. Possible transformations. \\label{Oly_DTW_SH_out_rmd_Trans}"}
...
Direct biomass data was only available for Deanza, which estimated `r round(NPD_total_DTW, 2)` g/m^2^ of bivalve dry tissue weight (DTW). 
*O. lurida* had the highest DTW with `r round(NPD_species_DTW[NPD_species_DTW$Species=="Ostrea lurida",2],2)` g/m^2^, followed by *Mytilus galloprovincialis* (`r round(NPD_species_DTW[NPD_species_DTW$Species=="Mytilus galloprovincialis",2],)` g/m^2^), an unknown mussel (`r round(NPD_species_DTW[NPD_species_DTW$Species=="Unknown mussel",2],2)` g/m^2^), *Adula diegensis* (`r round(NPD_species_DTW[NPD_species_DTW$Species=="Adula diegensis",2],2)` g/m^2^), and *Musculista senhousia* (`r round(NPD_species_DTW[NPD_species_DTW$Species=="Musculista senhousia",2],2)` g/m^2^).
San Rafael survey data did not directly assess biomass, but did include shell length measurements which were used to estimate DTW of *O. lurida* (`r round(SR_total_DTW,3)` g/m^2^) from the Deanza shell length:DTW model (model equation here). 
We estimated San Rafael had `r round(SR_species_DTW$total,2)`.

```{r Bivalve_biomass_bargraph, echo=FALSE, fig.align="center", fig.asp=0.618, fig.width=6, warning= F, message= F, fig.keep='all', dev='pdf', out.width="70%", fig.cap="Estimated biomass of \\textit{O. lurida} habitat at Newport Deanza and San Rafael sites. Data for Deanza was collected by the Zacherl lab in May 2018, the relationship between \\textit{O. lurida} shell length and dry tissue weight (DTW) from the Deanza 2018 survey was used to estimate DTW at San Rafael with shell length data collected by the Zabin lab in November 2017. \\label{Bivalve_Den_Biomass}"}
