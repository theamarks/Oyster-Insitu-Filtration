...
Final_table <- Master_analysis_table %>%
  mutate(Date = mdy(Date),
         ...
         Experiment = ifelse(grepl("Neg Control*", Master_analysis_table$Experiment), gsub("Neg Control*", "Ctrl", Master_analysis_table$Experiment), "Fltr"),
  ...
  dplyr::select(Site, Date, Experiment, pcnt_Chl_rmvd, L_hr_m2, Temp_C_Up, Sal_ppt_Up, 
...
At Deanza we found a significant relationship between *O. lurida* shell length and DTW (ln(y) = `r round(coef(Oly_logDTW_lm_sum)[1,1],digits=2)` `r round(coef(Oly_logDTW_lm_sum)[2,1],digits=3)`x, R^2^ = `r round(Oly_logDTW_lm_sum$r.squared,digits=2)`, *p* = `r round(coef(Oly_logDTW_lm_sum)[2,4],digits=3)`).
