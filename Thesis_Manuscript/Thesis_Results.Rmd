...
At Deanza we found a significant relationship between *O. lurida* shell length and DTW (ln(y) = `r round(coef(OlyDTW_loglog_sum)[1,1],digits=2)` `r round(coef(OlyDTW_loglog_sum)[2,1],digits=3)`x, R^2^ = `r round(OlyDTW_loglog_sum$r.squared,digits=2)`, *p* = `r round(coef(OlyDTW_loglog_sum)[2,4],digits=3)`).
