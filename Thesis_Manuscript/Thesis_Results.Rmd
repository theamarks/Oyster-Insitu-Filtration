...
temp2 = missForest(Final_table[,-c(1,2,3)])$ximp # This is the imputation step? -AM
...
######## Descriptive Filtration (as a function of turbitidy, salinity ect ect) --> RANDOM FOREST MODEL
...
model.rf
plot(model.rf)
y.hat = predict(model.rf,newdata=test1) #predict new clearance values based on original data set values
res = test1$Clearance - y.hat # residuals?
...
model.rf$importance[order(model.rf$importance)] # trying to order variable importance in decending order
print(importance(model.rf))
