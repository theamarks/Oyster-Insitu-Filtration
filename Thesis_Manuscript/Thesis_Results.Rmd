...
Percent Chl $\alpha$ removed at the San Rafael *O. lurida* reef (n = `r nrow(SR_filter_sum)`) averaged `r round(mean(SR_filter_sum$pcnt_Chl_rmvd),1)`\% (SD = `r round(sd(SR_filter_sum$pcnt_Chl_rmvd),2)`) (Figure \ref{ChlRmd_Site_boxplot}) and recorded `r round(SR_NC_sum$pcnt_Chl_rmvd,1)`\% in the single control trial. 
...
4 data values (2 TPM and 2 OC) values are missing from our data set, representing `r round((4/336)*100, digits = 0)`% of the data set. 
