...
# convert TPM, PIM, POM from g/L to mg/L
Master_analysis_table  <- Master_analysis_table %>% 
  mutate(Avg_TPM = Avg_TPM*1000,
         Avg_PIM = Avg_PIM*1000,
         Avg_POM = Avg_POM*1000,
         Avg_OC_Ratio = round(Avg_OC_Ratio,2))
...
Avg_TPM_summary_table <- Avg_TPM_summary_table %>% 
  # convert g/L to mg/L like Gray and Langdon 2018
  mutate(Avg_TPM = Avg_TPM*1000,
         Avg_PIM = Avg_PIM*1000,
         Avg_POM = Avg_POM*1000,
         Avg_OC_Ratio = round(Avg_OC_Ratio, 2))
...
The mean difference between upstream and downstream Chl $\alpha$ (paired by water travel time) was compared using a single sided t-test. 
Three filtraiton trials at San Rafael recorded significantly lower (p < 0.05) Chl $\alpha$ downstream of the *O. lurida* shell bags than Chl $\alpha$ upstream ( Table X).
One filtration trial (2018/7/17) and the negative control did not show a significant difference (p > 0.05) reduction of Chl $\alpha$ over the habitat from the upstream to downstream positions. 
At Morro Bay two of the four filtration trials showed a significant reduction in Chl $\alpha$ across the *C. gigas* aquaculture lines. 
Two filtration trials and the single negative control trial did not show Chl $\alpha$ removal.
Newport Shellmaker had significant Chl $\alpha$ reduction across the *O. lurida* shellbed at one of the four filtration trials.
The remaining two filtration trials and the negative control trial did not have significant Chl $\alpha$ reduction.
Five of the ten filtration trials at Newport Deanza recorded significant reduction of Chl $\alpha$ from the upstream to downstream measurements. 
Five filtraiton trials and the three negative trials did not have significant removal of Chl $\alpha$. 
See table X for statistical results of each trial. 

### Percent chlorophyll $\alpha$ removal

Percent Chl $\alpha$ removal at the San Rafael *O. lurida* reef from `r nrow(SR_filter_sum)` field days ranged from `r round(min(SR_filter_sum$pcnt_Chl_rmvd),1)`\% to `r round(max(SR_filter_sum$pcnt_Chl_rmvd),1)`\% and recorded `r round(SR_NC_sum$pcnt_Chl_rmvd,1)`\% in the single negative control over bare mudflat (Figure \ref{FR_ChlRmd_Site_boxplot}). 
...
Newport Deanza ranged from `r round(min(NPD_filter_sum$L_hr_m2),1)` to `r round(max(NPD_filter_sum$L_hr_m2),1)` Lhr^-1^m^2^ (Figure \ref{FR_ChlRmd_Site_boxplot}).
...
Final_table <- Master_analysis_table %>%
  mutate(Date = mdy(Date),
         ...
         Avg_TPM = round(Avg_TPM*1000, digits = 4), # convert g to mg
...
colnames(Final_table) <- c( "Site", "Date", "Experiment", "Temp ($\\textdegree$C)",
                            ...
                            "Chl $\\alpha$ up ($\\mu$g/L)", "Chl $\\alpha$ down ($\\mu$g/L)", 
...
print(sumtable_latex, booktabs=TRUE, caption.placement="top")
...
```{r Dr Nichols code, eval=FALSE, include=FALSE}
#In our data we are missing 4 out of 400ish values.  
...
Corr_Chl_diff_graph <- ggplot(Corr_chl_diff_data, aes(correction_noabs, filter_chl_diff)) +
  geom_point() +
  #geom_smooth(method = lm) + # fitted linear model
  theme_classic() + 
  xlim(-3,3) + # even axes
  ylim(-3,3) +
  ...
  geom_text(aes(x=correction_noabs + .12,label=dummie)) # 1-1 reference line
Corr_Chl_diff_graph     

Corr_chl_diff_data[c(9,21,23),]
## Value used to correct Filtration values = abs(correction_noabs/2)
sbs_filterdiff_lm <- lm(data = Corr_chl_diff_data, filter_chl_diff~correction_noabs)
summary(sbs_filterdiff_lm)
...
temp1 = Final_table[,c(1,2,3)]
temp2 = missForest(Final_table[,-c(1,2,3)])$ximp
...
#Descriptive Filtration (as a function of turbitidty, salinity ect, oyster only)
...
exp = test1[test1$Experiment=="Filtration",]
control = test1[test1$Experiment!="Filtration",]
t.test(exp$Clearance,control$Clearance,alternative="greater")

#Descriptive Filtration (as a function of turbitidy, salinity ect ect)


model.rf = randomForest(Clearance~Temp+Salinity+Turbidity+TPM+OC+Site,data=test1,ntree=100,mtry=3,control=rpart.control(minsplit=2,cp=.05))
y.hat = predict(model.rf,newdata=test1)
res = test1$Clearance - y.hat
r.sq = (var(test1$Clearance) - var(res))/var(test1$Clearance)
r.sq
model.rf$importance[order(model.rf$importance)]
...
```{r FR_ChlRmd_Site_boxplot, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.show="hold", fig.asp=0.618, fig.width=4.3, out.width="50%", message= F, fig.cap="The habitat level filtration rates and percent chlorophyll removed of all filtration trials across sites. \\label{FR_ChlRmd_Site_boxplot}"}
...
```{r FR_ChlRmd_Site_posFilter_boxplot, echo=FALSE, fig.keep = 'all', dev = 'pdf',fig.show="hold", fig.align="default", fig.asp=0.618, fig.width=4.3, out.width="50%", message= F, fig.cap="The habitat level filtration rates and percent chlorophyll removed of filtration trials (positive values only) across sites. \\label{FR_ChlRmd_Site_posFilter_boxplot}"}
...
```{r FR_ChlRmd_TPM_point, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.show="hold", fig.show = "hold", fig.align="default", fig.asp=0.618, fig.width=4.3, out.width="50%", warning = F, fig.cap="The habitat level filtration rates and percent chlorophyll removed of positive filtration trials in response to seston total particulate matter (TPM) . \\label{FR_ChlRmd_TPM_point}"}
...
```{r NPD_FRPos_TPM_graph, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.show="hold", fig.show = "hold", fig.align="default", fig.asp=0.618, fig.width=4.3, out.width="50%", message= F, fig.cap="The positive habitat level filtration rates in response to seston total particulate matter (TPM) at the Deanza site in Newport Bay, California. \\label{NPD_FRPos_TPM_graph}"}
...
Total Particulate Matter (TPM) varied widely among bays, while Organic Content (OC) was relatively similar among bays (Figure \ref{TPM_OC_graphs}). 
Both Newport Bay sites had overlapping TPM, while Morro Bay and San Rafael had distictly separate TPM ranges. 
More reportin ranges here?

```{r TPM_OC_graphs, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align="center", fig.show = "hold", fig.asp=0.618, fig.width=6, out.width = "70%", message= F, fig.cap="The relationship between seston total particulate matter (TPM) and seston organic content (OC) across all sites and all filtration and negative control trials. \\label{TPM_OC_graphs}"}
...
```{r TPM_OC_Sites_graphs, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align="default", fig.asp=0.618, fig.width=8, message= F, fig.cap="The relationship between seston total particulate matter (TPM) and seston organic content (OC) across all sites and all trials. \\label{TPM_OC_Sites_graphs}"}
...
```{r NPD_Oly_DTW_length_LM_logy, eval=FALSE, fig.height=6, fig.width=6,  include=FALSE}
...
```{r NPD_Oly_DTW_length_LM_loglog, eval=FALSE, fig.height=6, fig.width=6,  include=FALSE}
...
```{r NPD_Bivalve_SH_DTW, echo=FALSE, fig.align="center", fig.asp=0.618, fig.keep='all', fig.width=8.6, dev='pdf', out.width="100%", fig.cap="The biomass of bivalves excavated from Deanza in Newport Bay \\label{NPD_Bivalve_SH_DTW}"}
...
```{r NPD_Bivalve_SH_DTW_outrmd, echo=FALSE, fig.align="center", fig.asp=0.618, fig.cap="The biomass of bivalves excavated from Deanza in Newport Bay \\label{NPD_Bivalve_SH_DTW_outrmd}", fig.keep='all', fig.width=8.6, dev='pdf', out.width="100%"}
# Remove outlier and negative DTW value
...
```{r Bivalve_Den_biomass_bargraph, echo=FALSE, fig.align="center", fig.asp=0.618, fig.width=6, warning= F, message= F, fig.keep='all', dev='pdf', out.width="100%", fig.cap="The density of bivlaves at Deanza Newport, California surveyed May 2018. \\label{Bivalve_Den_Biomass}"}
...
```{r T_test_table, fig.height=5, fig.width=6, include=FALSE}
