...
The Random Forest model that included all filtration and control trials indicated that temperature had the highest relative importance to the model (`r round(Temp_rel_import, 1)`\%), followed by OC (`r round(OC_rel_import, 1)`\%), turbidity (`r round(Turb_rel_import, 1)`\%), TPM (`r round(TPM_rel_import, 1)`\%), salinity (`r round(Sal_rel_import, 1)`\%), and site (`r round(Site_rel_import, 1)`\%) respectively.
Filtration and control trials adjusted by the Random Forest model were not significantly different (two-sample t(`r t_test_adj_df`) = `r round(t_test_adj$statistic, digits = 2)`, p = `r round(t_test_adj$p.value, digits = 2)`).
