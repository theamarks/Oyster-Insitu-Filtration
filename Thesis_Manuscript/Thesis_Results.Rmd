...
colnames(Final_table) <- c( "Site", "Date", "Experiment", "Temp ($\\textdegree$C)","Salinity (ppt)", "Turbidity (NTU)", "TPM (mg/L)",
                           "OC", "Chl $\\alpha$ up ($\\mu$g/L)", "Chl $\\alpha$ down ($\\mu$g/L)", 
                           "Depth (m)","Distance (m)", "Water veolcity (m/s)", "% Chl $\\alpha$ removal", "Clearance rate (L/hr/$m^2$)")
# create xtable
...
```{r FR_ChlRmd_TPM_point, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.show="hold", fig.show = "hold", fig.align="default", fig.asp=0.618, fig.width=4.3, out.width="50%", warning = F, fig.cap="The habitat level filtration rates and percent chlorophyll removed of positive filtration trials in response to seston total particulate matter (TPM) . \\label{figurelabel}"}
...
```{r NPD_FRPos_TPM_graph, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.show="hold", fig.show = "hold", fig.align="default", fig.asp=0.618, fig.width=4.3, out.width="50%", message= F, fig.cap="The positive habitat level filtration rates in response to seston total particulate matter (TPM) at the Deanza site in Newport Bay, California. \\label{figurelabel}"}
