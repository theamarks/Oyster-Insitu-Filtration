...
Newport Shellmaker had significant Chl $\alpha$ reduction across the *O. lurida* shellbed at two of the four filtration trials.
...
Final_table <- Master_analysis_table %>%
  ...
  dplyr::select(Site, Date, Experiment, Temp_C_Up, Sal_ppt_Up, Turbidity_NTU_Up, Avg_TPM, 
          Avg_OC_Ratio, Chl_ug_L_Up, Chl_ug_L_Down, avg_depth_m, d_bw_sondes_m, avg_m_hr, 
...
colnames(Final_table) <- c( "Site", "Date", "Experiment", "Temp ($\\textdegree$C)","Salinity (ppt)", "Turbidity (NTU)", "TPM (mg/L)",
                           ...
                           "Depth (m)","Distance (m)", "Water veolcity (m/hr)", "% Chl $\\alpha$ removal", "Clearance rate (L/hr/$m^2$)")
# create xtable
