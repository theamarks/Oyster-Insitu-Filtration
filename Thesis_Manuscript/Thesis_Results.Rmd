...
OC was significant among sites OC (*F*(`r OC_aov_values[[1,2]]`, `r OC_aov_values[[2,2]]`) = `r round(OC_aov_values[[1,5]],2)`, *p* = `r round(OC_aov_values[[1,6]],2)`), but the post-hoc Tukey HSD did not reveal significant differences among sites. 
Therefore, I use a less conservative post-hoc analysis, the Kewman-Keuls method, and found that OC was significantly different between Shellmaker and Deanza (*p* = `r round(NK_oc$Site[[3,4]],2)`).
...
The residuals of this random forest regression adjusted the HCR values to account for water quality variables and site. 
...
Total Particulate Matter (TPM) was significantly related to seston OC when sites are combined  (Figure \ref{TPM_OC_model}). 
