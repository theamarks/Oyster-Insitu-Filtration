...
Negative control trials measured `r round(SR_NC_sum$L_hr_m2,1)`, `r round(MB_NC_sum$L_hr_m2,1)`, and `r round(NPSM_NC_sum$L_hr_m2,1)` Lhr^-1^m^2^ for San Rafael, Morro Bay, and Shellmaker respectively.
Newport Deanza's three consecutive negative control trials averaged `r round(mean(NPD_NC_sum$L_hr_m2))` Lhr^-1^m^2^ (n = `r nrow(NPD_NC_sum$L_hr_m2)`, SD = `r round(sd(NPD_NC_sum$L_hr_m2),1)`).
...
Northern San Francisco Bay (San Rafael site) TPM averaged `r round(mean(SR_TPM_OC$Avg_TPM_mg_L),2)` mg/L (n = `nrow(SR_TPM_OC)`, SD = `round(sd(SR_TPM_OC$Avg_TPM_mg_L),2)`, and Morro Bay TPM averaged `r round(mean(MB_TPM_OC$Avg_TPM_mg_L),2)` mg/L (n = `nrow(MB_TPM_OC)`, SD = `round(sd(MB_TPM_OC$Avg_TPM_mg_L),2)`.
Newport Bay (Deanza and Shellmaker sites) TPM averaged `r round(mean(NP_combo_TPM_OC$Avg_TPM_mg_L),3)` mg/L (n = `nrow(NP_combo_TPM_OC)`, SD = `round(sd(NP_combo_TPM_OC$Avg_TPM_mg_L),2)`.
