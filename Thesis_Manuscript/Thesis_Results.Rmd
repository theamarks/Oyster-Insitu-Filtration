...
header-includes: 
  \usepackage{setspace}\doublespacing\usepackage{rotating}\usepackage{booktabs}
output: 
  pdf_document:
    fig_caption: yes
...
Filtration trials at San Rafael were conducted on the ebb and flood tides surrouning low tide.
...
### Move to methods? 
Two field days are missing TPM and OC values from our data set, representing `r round((4/336)*100, digits = 0)`% of the data set. 
Removing these incomplete observations is costly for our data set where we have a limited number of observations with a large number of variables. 
...
Three extreme observations were removed from the data set based on a correction-to-signal comparison (Shellmaker 2020/6/9, Shellmaker 2020/5/22, Deanza 2020/4/17) (Figure \ref{SbsCorr_ChlDiff_graph).

We used a fitted a Random Forest model to predict HCR from temperature, salinity, turbidity, TPM, OC, and site.
...
```{r Oly_DTW_SH_outlier_rmd, echo=FALSE, fig.keep = 'all', dev = 'pdf', warning=F, message=F, fig.align="center", out.width = "70%", fig.asp=0.618, fig.width=6, fig.cap="The relationship between \\*O. lurida* shell length and its dry tissue weight (DTW) at two locations, Newport Bay, California (Zacherl) and Yaquina Bay, Oregon (Gray). The Newport Bay data was collected at the Deanza restoration site and outlier data (outside mean +- 2.2 SD) and negative values are excluded. \\label{Oly_DTW_SH_outlier_rmd}"}
...
*O. lurida* had the highest DTW with `r NPD_species_DTW[NPD_species_DTW$Species=="Ostrea lurida",2]` g/m^2^, followed by *Mytilus galloprovincialis* (`r NPD_species_DTW[NPD_species_DTW$Species=="Mytilus galloprovincialis",2]` g/m^2^), an unknown mussel (`r NPD_species_DTW[NPD_species_DTW$Species=="Unknown mussel",2]` g/m^2^), *Adula diegensis* (`r NPD_species_DTW[NPD_species_DTW$Species=="Adula diegensis",2]` g/m^2^), and *Musculista senhousia* (`r NPD_species_DTW[NPD_species_DTW$Species=="Musculista senhousia",2]` g/m^2^).
