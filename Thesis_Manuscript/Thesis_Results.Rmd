...
colnames(Final_table) <- c( "Site", "Date", "Experiment", "Temp ($\\textdegree$C)","Salinity (ppt)", "Turbidity (NTU)",
                            "TPM(mg/L)", "OC", "Chl $\\alpha$ up ($\\mu$g/L)", "Chl $\\alpha$ down ($\\mu$g/L)", 
                           "Depth (m)","Distance (m)", "Water veolcity (m/s)", "% Chl $\\alpha$ removal", 
                           "Clearance rate (L/hr/$m^2$)")
# create xtable
...
```{r Bivalve_Density, echo=FALSE, fig.align="center", fig.asp=0.618, fig.width=6, fig.keep='all', dev='pdf', out.width="70%", fig.cap="The density of bivlaves at Deanza Newport, California surveyed May 2018. \\label{Bivalve_Density}"}
...
```{r Oly_DTW_SH_out_rmd_Trans, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align="default", fig.asp=0.618, fig.width=8, fig.cap="The relationship between *O. lurida* shell length and its dry tissue weight (DTW) in Newport Bay, California at the Deanza restoration site. Possible transformations. \\label{Oly_DTW_SH_out_rmd_Trans}"}
...
```{r Oly_TWW_DTW_outRmd, echo=FALSE, fig.keep = 'all', dev = 'pdf', fig.align="default", fig.asp=0.618, fig.width=8, message = F, fig.cap="The relationship between *O. lurida* total wet weight (TWW) and its dry tissue weight (DTW) in Newport Bay, California at the Deanza restoration site. Blue dashed lines represent the relationship found by Gray and Langdon (2018) in Yaquina Bay, Oregon Olympia oysters (DTW = 0.0044*TWW - 0.043) \\label{Oly_TWW_DTW_outRmd}"}
