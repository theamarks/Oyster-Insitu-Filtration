...
createSbsDensityPlot = function(adistrubution, aSbs_stat_summary, aFileName)
{
  ...
  Sbs_stats_plot = aSbs_stat_summary %>% 
   dplyr::select(Sample_sbs_Count, Mean_sbs_Chl_diff, SD_sbs_Chl_diff, SE_sbs_Chl_diff) %>% 
   #mutate_if(is.numeric, round, 3) %>% 
   rename("n" = Sample_sbs_Count,  
  ...
  distrubution_plot <- ggplot(data = adistrubution, aes(x = Chl_ug_L)) +
    ...
    annotate("text", x = aSbs_stat_summary$Mean_sbs_Chl_Down - 0.05, 
             y = 0, label = bquote(bar(x) ~ .(round(as.numeric(aSbs_stat_summary$Mean_sbs_Chl_Down), 2))),
              vjust = "top", hjust = "right", color = wes_palette("Cavalcanti1")[3]) +
    # add mean Up next to dashed line
      annotate("text", x = aSbs_stat_summary$Mean_sbs_Chl_Up - 0.05, 
              y = Inf, label = bquote(bar(x) ~ .(round(as.numeric(aSbs_stat_summary$Mean_sbs_Chl_Up), 2))),
