...
# library(flexdashboard) # knit html dashboard output template
...
# library(cowplot) # arrange multiple plots or objects in single plot
...
manual_correction_df %<>%
  dplyr::mutate(Correction_Start_Time = as_hms(Correction_Start_Time),
                Correction_End_Time   = as_hms(Correction_End_Time))
```
...
for(file in 1 : length(all_files))
{
  ...
  {  
   ...
   # one_file_stats = calculateErrorStats(one_file, file_name, manual_correction_df, error_directory)
    one_file_stats = summarizeSbsCorrectionValues(one_file, file_name)
    sbs_stats_summary = rbind(one_file_stats, sbs_stats_summary) 
  }
  
}

# Save the SBS Statistics summary
fwrite(sbs_stats_summary, file.path(error_directory, "Sbs_stats_summary.csv"))
```
